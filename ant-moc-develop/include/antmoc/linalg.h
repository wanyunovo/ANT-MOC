...
namespace antmoc
{
  ...
  struct DomainCommunicator
  {
    ...
    概念：域分解（Domain Decomposition）
    将整个计算区域划分成多个子域，每个进程负责一个子域
    例如：100×80×60 的网格，用 4×2×2 的进程网格分解
    X 方向 4 个进程：每个进程负责 25 列
    Y 方向 2 个进程：每个进程负责 40 行
    Z 方向 2 个进程：每个进程负责 30 层
    总共 16 个进程并行计算
    */
    int _num_domains_x; // X方向域的总数 4
    ...
    每个域负责的CMFD网格数是：
    X方向 _local_num_x：100 / 4 = 25 列
    Y方向 _local_num_y：80 / 2 = 40 行
