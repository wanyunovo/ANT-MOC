...
namespace antmoc
{
  ...
  protected:
     ...
     * 极角正弦
     * 二维数组
     * _sin_thetas[azim][polar] = sin(theta_p)
     */
    std::vector<DoubleVec> _sin_thetas;

    /** An array of the quadrature polar angles
     * 极角值
     * 某些求积组里，极角分布可能依赖方位角，因此用二维数组（而不是单纯的一维）。
     * _thetas[azim][polar] = theta_p
     */
    ...
    */
    ...
    {
      size_t c_azim_index = _num_azim / 2 - azim_index - 1; // 设置对称的下标索引
      size_t s_polar_index = _num_polar - polar_index - 1;  // 设置对称的下标索引
  ...
  public:
    ...
    const std::vector<DoubleVec> &getTotalWeights() const;
