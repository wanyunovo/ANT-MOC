...
#define _boundary_flux(i, j, pe) (_boundary_flux[(i) * 2 * _fluxes_per_track + (j) * _fluxes_per_track + (pe)])
#define _start_flux(i, j, pe) (_start_flux[(i) * 2 * _fluxes_per_track + (j) * _fluxes_per_track + (pe)])
...
#define scatter_sources(r, e) (scatter_sources[(r) * _num_groups + (e)])
  ...
  protected:
    ...
    TrackGeneratorPtr _track_generator_smart;
    ...
    /** Boolean for whether to check unphysical cross-sections */
    bool _check_xs;
     ...
     * 用于计算过程中的临时缓存，避免在循环中反复分配内存。
     */
    std::vector<FP_PRECISION *> _groupwise_scratch;

    /** Temporary scratch pad for region-wise computations 
     * 用于按区域计算的临时缓存。
    */
  ...
  public:
    ...
    Solver(TrackGeneratorPtr track_generator = NULL);
    virtual ~Solver();
    ...
    void setCorrectXS(bool correct_xs);
    void setCheckXS(bool check_xs);
    void stabilizeTransport(double stabilization_factor,
                            stabilizationType stabilization_type = stabilizationType::DIAGONAL);

    /* Initial guesses for the flux */
    void setRestartStatus(bool is_restart);
    void setInitialSpectrumCalculation(double threshold);
    void setCheckXSLogLevel(log::level log_level);
    ...
    void printObjectStatistics(long tracks, long segments, long fsrs);
