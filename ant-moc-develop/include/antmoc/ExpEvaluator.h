...
namespace antmoc
{
  ...
  class ExpEvaluator
  {
    // 指数求值器类。它内部维护了一个数组，存储了x和e^(-x)的对应关系。
    /*
    1. 获取对应的求值器
    在扫描某条径迹时，你知道当前的方位角索引 (azimuthal index, a) 和 极角索引 (polar index, p)。
    // 获取当前角度对应的求值器
    ExpEvaluator* evaluator = _exp_evaluators[a][p];
    2. 计算光学长度
    计算光学长度 τ = Σ_t * L，其中 Σ_t 是总截面，L 是物理长度。
    3. 使用求值器计算指数值e^(-τ)
    FP_PRECISION exp_value = evaluator->computeExponential(τ);

    方法B：MOC 方程通常不仅需要e^(-τ)，还需要计算通量变化量
    */
