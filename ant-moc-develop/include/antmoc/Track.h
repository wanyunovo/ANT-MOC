...
namespace antmoc
{
  ...
  struct segment
  {
    ...
    /** The ID for the mesh surface crossed by the Track end point 轨迹终点所穿过的CMFD网格面的ID*/
    int _cmfd_surface_fwd;

    /** The ID for the mesh surface crossed by the Track start point 轨迹起点所穿过的CMFD网格面的ID*/
    int _cmfd_surface_bwd;

    /** The starting point of the segment relative to the FSR centroid */
    FP_PRECISION _starting_position[3]; 
    /*
     FP_PRECISION 不是一个 typedef 出来的类型，而是 CMake 在配置阶段加到编译宏里的别名。cmake/SetupUserOptions.cmake (lines 31-55) 里根据 ENABLE_DOUBLE_PRECISION 开关决定加上 -DFP_PRECISION=double 或 -DFP_PRECISION=float（同时还配置 CMFD 精度、线性代数容差等）。因此：
     默认启用双精度时，所有 FP_PRECISION 都会在预处理阶段替换成 double。
    如果你在配置时关掉 ENABLE_DOUBLE_PRECISION，它就会被替换成 float。
    所以 Track.h、Material.cpp 等文件里看到的 FP_PRECISION 实际上是“根据构建选项选出来的浮点类型占位符”。
     */
  ...
  protected:
    ...
    /** The Track's direction along the cyclic track or not */
    bool _reversed;
    ...
    std::vector<segment> _segments; // 轨迹包含的线段

    /** Number of segments recorded during volume calculation */
    int _num_segments; // 线段数量,主要用于3D的OTF方法,因为OTF方法不显式创建3D线段
    ...
    boundaryType _bc_fwd; // 沿fwd方向出射点的边界条件

    /** An enum to indicate whether the outgoing angular flux along this
     *  Track's "reverse" direction should be zeroed out for vacuum boundary
     *  conditions or sent to a periodic or reflective track. */
    boundaryType _bc_bwd; // 沿bwd方向出射点的边界条件

    /* Indices that are used to locate the track in the various track arrays
    我是第几个角（_azim_index）、这个角下第几条线（_xy_index）、在某条轨迹链里的第几节（_link_index）
     */
    int _azim_index;  // 方位角索引,确定属于第几个方位角
    long _xy_index;   // xy面上的索引,确定属于该角度的第几条轨迹
    long _link_index; // 在2D chain中的索引
    long _chain_index;

    /** Pointers to next, reflective, and periodic Tracks in the forward and
     *  reverse directions */
    long _track_next_fwd; // 周期或Interface边界,相接的下一条轨迹编号
    long _track_next_bwd;
    long _track_prdc_fwd; // 周期边界,相连的下一条轨迹编号
    long _track_prdc_bwd;
    long _track_refl_fwd; // 全反射边界,相连的下一条轨迹编号
     ...
     *  direction. */
    bool _next_fwd_fwd; /* true:这条轨迹沿fwd方向反射之后,方向恰好是相应反射轨迹的fwd方向 */
    bool _next_bwd_fwd; /* true:这条轨迹沿bwd方向反射之后,方向恰好是相应反射轨迹的fwd方向 */
    ...
    int _surface_in;  // 入射点所在的面的编号(0,1,3,4)
    int _surface_out; // 出射点所在的面的编号(0,1,3,4)
  ...
  public:
    ...
    void setLinkIndex(long index);
    void setChainIndex(long index);
  ...
  inline segment *Track::getSegment(int segment)
  {
    ...
    if (segment >= (int)_segments.size())
      log::ferror("Attempted to retrieve segment s = %d but Track only "
                  "has %d segments",
