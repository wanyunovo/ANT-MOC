...
namespace antmoc
{
   ...
   *          material input files. The method makes use of a static Material
  ...
  {
    if (_sigma_t == NULL)
      log::ferror("Unable to return Material %d's total "
                  "cross-section since it has not yet been set",
                  _id);
  ...
  {
    if (_sigma_s == NULL)
      log::ferror("Unable to return Material %d's scattering "
                  "cross-section since it has not yet been set",
                  _id);
  ...
  FP_PRECISION *Material::getSigmaA()
  {
    ...
    if (_num_groups <= 0)
      log::ferror("Unable to return Material %d's absorption "
                  "cross-section since it has %d groups",
                  _id, _num_groups);
  ...
  {
    if (_sigma_f == NULL)
      log::ferror("Unable to return material %d's fission "
                  "cross-section since it has not yet been set",
                  _id);
  ...
  {
    if (_nu_sigma_f == NULL)
      log::ferror("Unable to return Material %d's nu times fission "
                  "cross-section since it has not yet been set",
                  _id);
  ...
  {
    if (_chi == NULL)
      log::ferror("Unable to return Material %d's chi spectrum "
  ...
  FP_PRECISION Material::getSigmaTByGroup(int group)
  {
    if (_sigma_t == NULL)
      ...
      log::ferror("Unable to get sigma_t for group %d for Material "
  ...
  FP_PRECISION Material::getSigmaSByGroup(int origin, int destination)
  {
    if (_sigma_s == NULL)
      ...
      log::ferror("Unable to get sigma_s for group %d,%d for Material %d "
  ...
  FP_PRECISION Material::getSigmaAByGroup(int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to get sigma_a for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  FP_PRECISION Material::getSigmaFByGroup(int group)
  {
    if (_sigma_f == NULL)
      ...
      log::ferror("Unable to get sigma_f for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  {
    if (_nu_sigma_f == NULL)
      log::ferror("Unable to return Material %d's nu-fission "
                  "cross section since it has not yet been set",
                  _id);

    else if (group <= 0 || group > _num_groups)
      log::ferror("Unable to get nu_sigma_f for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  FP_PRECISION Material::getChiByGroup(int group)
  {
    if (_chi == NULL)
      ...
      log::ferror("Unable to get chi for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  FP_PRECISION Material::getFissionMatrixByGroup(int origin, int destination)
  {
    if (_fiss_matrix == NULL)
      buildFissionMatrix();
             ...
             origin > _num_groups || destination > _num_groups)
      log::ferror("Unable to get fission matrix for group %d,%d for "
  ...
  void Material::setNumEnergyGroups(const int num_groups)
  {
    ...
    if (num_groups < 1)
      log::ferror("Unable to set the number of energy groups for "
                  "material %d to %d",
                  _id, num_groups);
    ...
    _sigma_t = new FP_PRECISION[_num_groups]();               // 总截面
    _sigma_f = new FP_PRECISION[_num_groups]();               // 裂变截面
    _nu_sigma_f = new FP_PRECISION[_num_groups]();            // 裂变截面乘以 ν（每次裂变产生的中子数）的值。
    _chi = new FP_PRECISION[_num_groups]();                   // 裂变中子能谱
    _sigma_s = new FP_PRECISION[_num_groups * _num_groups](); // 散射截面
  }
  ...
  void Material::setSigmaT(double *xs, int num_groups)
  {
    ...
    if (_num_groups != num_groups)
      log::ferror("Unable to set sigma_t with %d groups for Material "
                  "%d which contains %d energy groups",
                  num_groups,
                  _id, _num_groups);
    ...
    for (int i = 0; i < _num_groups; i++)
    {
      ...
      {
        log::verbose_once("Overriding zero sigmaT in group {} for Material '{}'(id={}) "
                          "with 1E-10",
                          i, _name, _id);
  ...
  void Material::setSigmaTByGroup(double xs, int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set sigma_t for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
   ...
   * @details The array should be passed to the code as a 1D array in
   ...
   *  这张矩阵的 (origin→destination) 元素给出粒子从某个能群被散射到另一个能群的概率/截面。按列主序，列代表“起点能群”，行代表“落点能群”。
   */
  void Material::setSigmaS(double *xs, int num_groups_squared)
  {

    if (_num_groups * _num_groups != num_groups_squared)
      log::ferror("Unable to set sigma_s with %f groups for Material %d "
                  "which contains %d energy groups",
  ...
  void Material::setSigmaSByGroup(double xs, int origin, int destination)
  {
    ...
    if (origin <= 0 || destination <= 0 || origin > _num_groups ||
      ...
      log::ferror("Unable to set sigma_s for group %d -> %d for Material %d"
                  " which contains %d energy groups",
                  origin, destination, _id, _num_groups);
  ...
  void Material::setSigmaF(double *xs, int num_groups)
  {
    ...
    if (_num_groups != num_groups)
      log::ferror("Unable to set sigma_f with %d groups for Material "
                  "%d which contains %d energy groups",
                  num_groups, _id,
                  _num_groups);
  ...
  void Material::setSigmaFByGroup(double xs, int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set sigma_f for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  void Material::setNuSigmaF(double *xs, int num_groups)
  {
    ...
    if (_num_groups != num_groups)
      log::ferror("Unable to set nu_sigma_f with %d groups for Material %d "
                  "which contains %d energy groups",
  ...
  void Material::setNuSigmaFByGroup(double xs, int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set nu_sigma_f for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  {
    if (_num_groups != num_groups)
      log::ferror("Unable to set chi with %d groups for Material "
                  "%d which contains %d energy groups",
                  num_groups, _id, _num_groups);
  ...
  void Material::setChiByGroup(double xs, int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set chi for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
   ...
   * @details The absorption cross section is computed at runtime when needed,
  ...
  void Material::setSigmaAByGroup(double xs, int group)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set sigma_a for group %d for Material "
                  "%d which contains %d energy groups",
                  group, _id, _num_groups);
  ...
  void Material::buildFissionMatrix()
  {
    ...
    if (_num_groups == 0)
      ...
      log::ferror("Unable to build Material %d's fission matrix "
                  "since its nu-fission cross-section has not been set",
                  _id);
    else if (_chi == NULL)
      log::ferror("Unable to build Material %d's fission matrix "
                  "since its chi spectrum has not been set",
                  _id);
  ...
  void Material::alignData()
  {
    ...
    if (_num_groups <= 0)
      log::ferror("Unable to align Material %d data since the "
  ...
  std::string Material::toString()
  {
    ...
    string << "Material id = " << _id << ", name = " << _name;
  ...
  /**
   * @breif Converts a string-formed id into integer
   * @details This method can only handle strings in format 'M-N',
   *          e.g. '1-1', '90-289'. The string will be separated into
   *          two parts. The second bit is a flag to indicate whether
   *          the number is split or not. M will be stored in the higher
   *          10 bits of the integer, and N will be on the lower 20 bits
   *          of the integer.
   *          0|0|        M  |              N
   *          s|0|000000 0000|0000 00000000 00000000
   *          M ranges from 0 to 2^10-1, N ranges from 0 to 2^20-1.
   *          So the maximum number of materials is 1023-1048575.
   */
  std::int32_t Material::convertStringToId(std::string id_string)
  {
    std::ostringstream ss;
    const char *str = id_string.c_str();
    std::int32_t upper = 0;

    for (auto c = str; *c; c++)
    {
      // If it is the first time to split the id
      if (*c == '-' && (upper >> 30) == 0)
      {
        // Put the first part on bit 3~12, 10 bits in total
        try
        {
          upper = stoi(ss.str()) << 20;
        }
        catch (std::exception &)
        {
          return -1;
      ...
      if (!std::isdigit(*c)) // Handle non-digit chars
        return -1;
      ss << *c;
    }

    /* If there is a '-', variable upper will be some value
     * the lower 15 bits will be added and returned */
    return upper + stoi(ss.str());
  }

  std::string Material::convertIdToString(std::int32_t id)
  {
    std::ostringstream ss;
    if (id >> 30)
    {
      id = (id << 2) >> 2; // Clear the flag
      ss << (id >> 20);
      ss << "-" << (id & 0xfffff);
    }
    else
