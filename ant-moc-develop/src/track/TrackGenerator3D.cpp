...
void TrackGenerator3D::setSegmentationZones(std::vector<double> zones) {
  ...
  if (zones.size() >= 2) {
    ...
    if (fabs(zones.at(0) - z_min) > TINY_MOVE)
      log::error("Segmentation zones must contain the Geometry minimum. "
                 ...
                 "Geometry minimum is {}.", zones.at(0), z_min);
    // 最后一个z高度要等于几何顶边的z坐标
    if (fabs(zones.at(zones.size()-1) - z_max) > TINY_MOVE)
      log::error("Segmentation zones must contain the Geometry maximum. "
                 ...
                 "Geometry maximum is {}.", zones.at(zones.size()-1), z_max);
  }
  else {
    log::error("Segmentation zones must contain the Geometry minimum "
  ...
  if (min_idx == max_idx) {  //这意味着所有的 zones 都不在当前计算域的范围内
...
void TrackGenerator3D::initialize3DTracks() {
  ...
  for (int a = 0; a < _num_azim/4; a++) {
    ...
    for (int p = 0; p < _num_polar/2; p++) {
      ...
      _num_z[a][p] = (int) ceil(module_width_z * _num_l[a][p] * tan(theta)
                                / length);
  ...
  log::finfo("RMS polar angle correction %f %%", sqrt(sum_correction)
...
int TrackGenerator3D::getFirst2DTrackLinkIndex(TrackChainIndexes* tci,
  ...
  else {  // Only ANT-MOC algorithms generate reflective cyclic tracks
...
int TrackGenerator3D::getFirst2DCyclicLinkIndex(TrackChainIndexes* tci,
  ...
  else if (map2DChainGid2Id(tci->_azim, tci->_x)
           >= getMyNum2DChains(tci->_azim)) {
  ...
  for (link_index = 0; link_index < links; link_index++) {
    ...
    l_sum += l_track;
    if (l_start <= l_sum) {
      if (l_sum - l_start < TINY_MOVE) {  // Almost on the boundary
        nudged = true;
        l_start += l_nudge;
      }
      else {  // A normal starting point
        break;
      }
    }
  }
...
void TrackGenerator3D::set3DTrackData(TrackChainIndexes* tci,
  ...
  while (true) {
    ...
    #ifdef USTB_
    end_of_chain = (fabs(dl_z - dl_xy) > TINY_MOVE && dl_z < dl_xy)
                    || link >= getNum2DLinks(tci->_azim) - 1
                    || tci->_link == link - first_link;
    #elif MIT_
    end_of_chain = dl_z < dl_xy
                    || xy >= _num_y[tci->_azim]
                    || tci->_link == link - first_link;
    #endif
...
void TrackGenerator3D::segmentizeExtruded() {
  ...
  if (mpi::isPrdTrackDecomposed()) {
    ...
    if (tot_num_efsrs != global_efsrs) {
      log::error("The number of extruded FSRs is inconsistant across processes: "
                 "Local = {}, Global = {}", tot_num_efsrs, global_efsrs);
    }
  }
  ...
  _geometry->initializeFSRVectors();  //CMFD关联FSR中的fsr_id, 以及_FSRs_to_CMFD_cells.at(fsr_id) = fsr->_cmfd_cell; fsr_id与CMFD单元相关联
...
void TrackGenerator3D::allocateTemporaryTracks() {
  ...
  /* Report memory usage */
  double size_mb = static_cast<double>(_max_num_tracks_per_stack) * _num_threads
                   * sizeof(Track3D) / (1 << 20);
...
double TrackGenerator3D::countTracksMemory(char format) {
    ...
    // by azim
    num = _num_azim/2.0;
    size = sizeof(_tracks_2D[0])
           + sizeof(_num_x[0]) + sizeof(_num_y[0])
           + sizeof(_num_chains[0]) + sizeof(_cyclic_length[0])
           + sizeof(_tracks_per_azim[0])
           + sizeof(_tracks_2D_chains[0])
           + sizeof(_dz_eff[0]) + sizeof(_dl_eff[0]) + sizeof(_num_z[0])
           + sizeof(_num_l[0]) + sizeof(_cum_tracks_per_stack[0])
           + sizeof(_cum_tracks_per_xy[0]) + sizeof(_tracks_per_stack[0])
           + sizeof(_first_lz_of_stack[0]);
#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    ...
    size = sizeof(Track)
           + sizeof(_cum_tracks_per_stack[0][0]) + sizeof(_cum_tracks_per_xy[0][0])
           + sizeof(_tracks_per_stack[0][0]) + sizeof(_first_lz_of_stack[0][0]);
    total_mem += num * size;
    ...
    size = sizeof(_dz_eff[0][0]) + sizeof(_dl_eff[0][0])
           + sizeof(_num_z[0][0]) + sizeof(_num_l[0][0]);
    total_mem += num * size;

    // by tracks and polar angles
    num = (double)getMyNum2DTracks() * _num_polar;
    size = sizeof(_cum_tracks_per_stack[0][0][0])
           + sizeof(_tracks_per_stack[0][0][0]) + sizeof(_first_lz_of_stack[0][0][0]);
    total_mem += num * size;

    // explicit 3D tracks
    if(_segment_formation == +segmentationType::EXPLICIT_3D)
        total_mem += _num_azim/2.0 * sizeof(_tracks_3D[0])
                     + getMyNum2DTracks() * sizeof(_tracks_3D[0][0])
                     + getMyNum2DTracks() * _num_polar * sizeof(_tracks_3D[0][0][0])
                     + getMyNum3DTracks() * sizeof(Track3D);
