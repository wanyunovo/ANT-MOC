...
namespace antmoc
{
  ...
  TrackGenerator3D::TrackGenerator3D(Geometry *geometry, int num_azim,
                                     ...
                                     double z_spacing) : TrackGenerator(geometry, num_azim, azim_spacing)
  {
  ...
  TrackGenerator3D::~TrackGenerator3D()
  {
    ...
    if (_contains_3D_tracks)
    {
      ...
      /* Delete 3D tracks */
      delete[] _tracks_per_stack[0][0];
      delete[] _first_lz_of_stack[0][0];
  ...
  /// \brief Count the number of 3D explicit segments.
  /// \return Total number of segments.
  long TrackGenerator3D::countNum3DExplicitSegments()
  {

    // After segmentize() or segmentizeExtruded(), this flag will be set to true.
    if (!containsSegments())
      log::error("Cannot count 3D explicit segments since they have not been generated.");

    log::verbose("Counting 3D explicit segments...");
...
#pragma omp critical
  ...
  /// \details The value is stored to avoid unnecessary re-caculation.
  /// \return The total number of 3D Track segments
  long TrackGenerator3D::getNum3DSegments()
  {

    if (_segments_counted)
    {
      // Avoid re-caculate the number of 3D segments.
      return _num_segments;
    }
    else
    {
      // Call subroutines to count segments.
      long num_3D_segments = 0;
      if (_segment_formation == +segmentationType::EXPLICIT_3D)
      {
        num_3D_segments = countNum3DExplicitSegments();
      }
      else
      {
        num_3D_segments = countNumSegments();
      }
  ...
  void TrackGenerator3D::setNumPolar(int num_polar)
  {
    ...
    if (num_polar < 0)
      log::ferror("Unable to set a negative number of polar angles "
                  "%d for the TrackGenerator.",
                  num_polar);

    if (num_polar % 2 != 0)
      log::ferror("Unable to set the number of polar angles to %d for the "
                  "TrackGenerator since it is not a multiple of 2",
                  num_polar);
  ...
  {
    if (spacing < 0)
      log::ferror("Unable to set a negative track z-spacing "
  ...
  void TrackGenerator3D::setSegmentationZones(std::vector<double> zones)
  {
    ...
    double z_min = _geometry->getGlobalMinZ();
    double z_max = _geometry->getGlobalMaxZ();
    if (zones.size() >= 2)
    {
      // 第一个z高度要等于几何底边的z坐标
      if (fabs(zones.at(0) - z_min) > TINY_MOVE)
        log::error("Segmentation zones must contain the Geometry minimum. "
                   "The first value of the segmentation heights is {} and the "
                   "Geometry minimum is {}.",
                   zones.at(0), z_min);
      // 最后一个z高度要等于几何顶边的z坐标
      if (fabs(zones.at(zones.size() - 1) - z_max) > TINY_MOVE)
        log::error("Segmentation zones must contain the Geometry maximum. "
                   "The last value of the segmentation heights is {} and the "
                   "Geometry maximum is {}.",
                   zones.at(zones.size() - 1), z_max);
    }
    else
    {
      log::error("Segmentation zones must contain the Geometry minimum "
    ...
    for (size_t i = 1; i < zones.size(); i++)
      if (zones.at(i) < zones.at(i - 1))
        log::error("Segmentation zones must be monotonically increasing. "
                   "Axial level {} is less than axial level {}",
    ...
    {
      // 用户输入的位于domain底边、底边以下的面都不考虑
      if (zones.at(i) - TINY_MOVE < domain_z_min)
        min_idx++;
      // 用户输入的位于domain顶边、顶边以上的面都不考虑
      // 这里解释一下为什么是小于号，因为比如zones.size()都不超过domain_z_max，那么max_idx自然而然就是zones.size()最大值，而不是0
      if (zones.at(i) + TINY_MOVE < domain_z_max)
        max_idx++;
    }

    log::debug("Set segmentation zones, min_idx = {}, max_idx = {}", min_idx, max_idx);

    /* Form segmentation heights */
    if (min_idx == max_idx)
    { // 这意味着所有的 zones 都不在当前计算域的范围内
      _segmentation_heights.push_back((domain_z_min + domain_z_max) / 2);
    }
    else
    {
      // 取的是每两个z高度的平均，包括domain底边的顶边。如果domain内有2个z高度，会存3个值
    ...
    // Print user-defined segmentation heights
    log::verbose("Found {} user-defined segmentation heights: [{}]",
                 _segmentation_heights.size(), stringutils::join(_segmentation_heights, ", "));
  }
  ...
  void TrackGenerator3D::useGlobalZMesh()
  {
    ...
    _global_z_mesh = _geometry->getUniqueZHeights(true); // 暂时没看
  }
  ...
  void TrackGenerator3D::retrieve3DTrackCoords(double *coords, long num_tracks)
  {
    ...
    if (num_tracks != NUM_VALUES_PER_RETRIEVED_TRACK * getNum3DTracks())
      log::ferror("Unable to retrieve the Track coordinates since the "
                  ...
                  getNum3DTracks(), NUM_VALUES_PER_RETRIEVED_TRACK * getNum3DTracks(), num_tracks);
                                                 ...
                                                 long num_segments)
  {
    ...
    if (num_segments != NUM_VALUES_PER_RETRIEVED_SEGMENT * getNum3DSegments())
      log::ferror("Unable to retrieve the Track segment coordinates since "
                  ...
                  getNum3DSegments(), NUM_VALUES_PER_RETRIEVED_SEGMENT * getNum3DSegments(), num_segments);
  ...
  void TrackGenerator3D::initializeTracks()
  {
    ...
    /* Initialize the 3D Track helpler arrays */
    initialize3DTracks();

    /* Initialize the 3D index arrays, save 3D tracks if needed*/
    ...
    log::finfo("Finished initialization of 3D tracks");

    _contains_3D_tracks = true;
  }

  /**
   * @brief Initialize 3D track helpler arrays
   * @details This method creates and initializes arrays of 3D track counts,
   *          spacings, and indices. These parameters are then used to
   *          correct themselves and update the quadrature.
   */
  void TrackGenerator3D::initialize3DTracks()
  {
    /* Make sure that the depth of the Geometry is nonzero */
    if (_geometry->getWidthZ() <= 0)
      log::ferror("The total depth of the Geometry must"
    ...
    if (!_contains_2D_tracks)
      log::ferror("Cannot initialize 3D tracks since the 2D tracks "
                  "have not been created");

    log::finfo("Initializing 3D tracks...");
    ...
    long offset;
    long num_stacks = getNum2DTracks() * _num_polar;
    int *cont_mem1 = new int[num_stacks];
    int *cont_mem2 = new int[num_stacks];
    ...
    for (int a = 0; a < _num_azim / 2; a++)
    {
      ...
      for (long i = 0; i < _num_x[a] + _num_y[a]; i++)
      {
        ...
        _tracks_per_stack[a][i] = &cont_mem1[offset + i * _num_polar];
        _first_lz_of_stack[a][i] = &cont_mem2[offset + i * _num_polar];
        for (int p = 0; p < _num_polar; p++)
        {
      ...
      offset += (_num_x[a] + _num_y[a]) * _num_polar;
    }
    ...
    for (int a = 0; a < _num_azim / 4; a++)
    {
      ...
      double length = getCyclicLength(a);
#elif MIT_
      double phi = _quadrature->getPhi(a);
      ...
      double length = width_y / sin(phi);
#endif

      // length /= (_geometry->getNumYModules() * _geometry->getNumXModules());
      ...
      for (int p = 0; p < _num_polar / 2; p++)
      {
        ...
        _num_l[a][p] = int(ceil(length * tan(M_PI_2 - theta) / _z_spacing));
        ...
        _num_z[a][p] = (int)ceil(module_width_z * _num_l[a][p] * tan(theta) / length);
        //_num_l[a][p] *= _geometry->getNumYModules();
        _num_z[a][p] *= _geometry->getNumZModules();

        /* Effective track spacing */
        // 2D轨迹链内包含的间距个数（所有模块）
        //_dl_eff[a][p] = width_y / (sin(phi) * _num_l[a][p]);
        // length *= (_geometry->getNumYModules() * _geometry->getNumXModules());
        _dl_eff[a][p] = length / _num_l[a][p];
        // 修正过的垂直方向间距（所有模块）
        _dz_eff[a][p] = width_z / _num_z[a][p];

        /* Evaluate the polar correction */
        theta_new = atan(_dl_eff[a][p] / _dz_eff[a][p]);
        ...
        theta = theta_new;
        _quadrature->setTheta(theta, a, p);
        _quadrature->setPolarSpacing(_dz_eff[a][p] * sin(theta), a, p);

        /* Save information for supplementary angles */
        int supp_azim = _num_azim / 2 - a - 1;
        _num_z[supp_azim][p] = _num_z[a][p];
        _dz_eff[supp_azim][p] = _dz_eff[a][p];
        _num_l[supp_azim][p] = _num_l[a][p];
        _dl_eff[supp_azim][p] = _dl_eff[a][p];

        /* Save information for complementary angles */
        int comp_polar = _num_polar - p - 1;
        _num_z[a][comp_polar] = _num_z[a][p];
        _dz_eff[a][comp_polar] = _dz_eff[a][p];
        _num_l[a][comp_polar] = _num_l[a][p];
        _dl_eff[a][comp_polar] = _dl_eff[a][p];

        _num_z[supp_azim][comp_polar] = _num_z[a][p];
        _dz_eff[supp_azim][comp_polar] = _dz_eff[a][p];
        _num_l[supp_azim][comp_polar] = _num_l[a][p];
        _dl_eff[supp_azim][comp_polar] = _dl_eff[a][p];
      }
    }

    /* Report polar correction information */
    log::finfo("RMS polar angle correction %f %%", sqrt(sum_correction) / _num_azim / _num_polar * 8 * 100);
    log::finfo("Max polar angle correction %f %%", max_correction * 100);
  }

  /**
   * @brief Initialize 3D index arrays, create 3D tracks if needed
   * @details This method initialize arrays for 3D track stack indexing.
   *          If explicit ray tracing is set, the method will create and
   *          save 3D tracks.
   */
  void TrackGenerator3D::set3DIndexArraysAndTracks()
  {
    // 类似于轨迹堆的概念，这里统计的是有多少堆轨迹链。
    // 每一堆轨迹链都有相同的极角，不同极角是不同的轨迹链堆。
    // 每个方位角中有_num_x[a] * _num_polar个轨迹链堆。
    ///< Local number of chain stacks
    long num_cstacks = getMyNum3DChainStacks();

    /* Create array of track chain indices */
    TrackChainIndexes *tcis = new TrackChainIndexes[num_cstacks];

    num_cstacks = 0;
    int num_chain_azims = getNumChainAzims();
    for (int a = 0; a < num_chain_azims; a++)
    {
      ...
      for (long x = 0; x < num_chains; x++)
      {
        for (int p = 0; p < _num_polar; p++)
        {
          tcis[num_cstacks]._azim = a;
          tcis[num_cstacks]._x = x;
          tcis[num_cstacks]._polar = p;
          num_cstacks++;
        }
      }
    }

    log::fresult("Total number of 3D Chains = %ld", getNum3DChains());
#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
      log::fverbose("Number of local 3D Chains = %ld", getMyNum3DChains());
#endif
     ...
     * l+ ->
     */
    // 返回后，_tracks_per_stack被赋值，这就是这个调用的目的（最后形参为false时）
    getCycleTrackData(tcis, num_cstacks, false);

    /* Allocate memory for 3D track stacks */
    // 里面有开关，如果使用EXPLICIT_3D，会在这个函数中给3D轨迹分配内存
    create3DTracksArrays();

    /* Initialize tracks in _tracks_3D array, save tracks */
    if (_segment_formation == +segmentationType::EXPLICIT_3D)
      getCycleTrackData(tcis, num_cstacks, true);
...
#pragma omp for
                                                 ...
                                                 Track3D *track_3D)
  {
    ...
    Track **chain_2D = _tracks_2D_chains[tci->_azim][tci->_x];
    Track *track_2D = chain_2D[0];
    ...
    // 是两个起点之前的长度，即，3D轨迹起点在lz平面上相对于2D轨迹链起点的距离
    double l_start = 0.0;
#ifdef USTB_
    double L = getCyclicLength(tci->_azim);
#elif MIT_
    double L = width_y / sin_phi;
#endif
    ...
    /* 3D轨迹起点在xy平面上相对于(_x_min,_y_min)的坐标
     * 如果phi是钝角，3D轨迹起点又不在link=0的2D轨迹上，那么x_ext<0.0
     */
    ...
    // The index of the first 2D Track link
    // For OpenMOC chains which can only be expanded along X
    // planes, link_index = link_x.
    // For ANT-MOC chains which should be expanded along
    // both X and Y planes, link_index = link_x + link_y.
    int link_index = abs(int(floor(x_ext / width_x)));
#ifdef USTB_
    // Expand the cyclic track along y planes
    link_index += abs(int(floor(y_ext / width_y)));
#endif

    // Compute x1, y1
    // Handle periodic and reflective BC respectively
    double x1, y1;
    if (!isReflectiveCyclic())
    {
      /* Get the starting x-coord */
      if (x_ext < 0.0) // 2D轨迹链穿出了左边界的情况
        x1 = fmod(x_ext, width_x) + _x_max;
      else // 2D轨迹链穿出了右边界的情况
        x1 = fmod(x_ext, width_x) + _x_min;

      /* Get the starting y-coord */
      y1 = fmod(y_ext, width_y) + _y_min;
    }
    else
    { // Only ANT-MOC algorithms generate reflective cyclic tracks
      /* Compute relative length */
      // 循环轨迹展开之后，最后一段轨迹在x边上，用l_rel_y;
      // 最后一段轨迹在y边上，用l_rel_x。总之，用较小的那一个。
      double l_rel_x = fmod(x_ext, width_x) / cos_phi;
      double l_rel_y = fmod(y_ext, width_y) / sin_phi;

      double l_rel = std::min(l_rel_x, l_rel_y);
      double l_track = chain_2D[link_index]->getLength();
      double scale = l_rel / l_track;

      /* Compute x- and y-coords */
      Point *p_start = chain_2D[link_index]->getStart();
      Point *p_end = chain_2D[link_index]->getEnd();

      /* Assume P_start is the starting point
       * P = P_start + scale * (P_end - P_start) */
      x1 = p_start->getX() + scale * (p_end->getX() - p_start->getX());
      y1 = p_start->getY() + scale * (p_end->getY() - p_start->getY());
    }
     ...
     *          /
     */
    // 和2D轨迹起止点的计算是类似的，需要注意的是，lz会取遍当前lz平面内的
    // 所有3D轨迹链，因此这里的起止z坐标都是整条轨迹链的坐标，不是链中某条
    // 轨迹的坐标。
    double z1;
    double z2;
    ...
    if (nudged)
    {
      // 反射性轨迹链上的轨迹不全是同一方位角，因此需要重新赋值
      phi = chain_2D[link_index]->getPhi();
      x1 -= 10 * TINY_MOVE * std::cos(phi);
      y1 -= 10 * TINY_MOVE * std::sin(phi);
    }

    /* Set the Track start point and ending z-coord if necessary*/
    if (track_3D != nullptr)
    {
  ...
  /// \brief Compute the starting coordinates of a 3D chain track, and the link
  ///        number of the 2D track where its start point reside on
  /// \details This method returns the link index of a 3D chain track. If a 3-D
  ///          track is passed, its starting point will be set.
  ///          As an alternative, this method only should be used for testing.
  /// \param tci the track chain index of a chain track
  /// \param track_3D the chain track, set to nullptr by default
  /// \return the link number of the 2D track where the track_3D start point
  ///         reside on
  int TrackGenerator3D::getFirst2DCyclicLinkIndex(TrackChainIndexes *tci,
                                                  Track3D *track_3D)
  {

#ifdef ENABLE_DEBUG_
    if (tci->_azim >= getNumChainAzims())
    {
      log::ferror("Azim index out of range: "
                  "num_chain_azims = %d"
                  ", tci_azim = %d, tci_x = %ld",
                  getNumChainAzims(), tci->_azim, tci->_x);
    }
    else if (map2DChainGid2Id(tci->_azim, tci->_x) >= getMyNum2DChains(tci->_azim))
    {
      log::ferror("Chain index out of range: "
                  "num_chain_azims = %d, num_chains = %ld"
                  ", tci_azim = %d, tci_x = %ld",
                  getNumChainAzims(), getMyNum2DChains(tci->_azim),
                  tci->_azim, tci->_x);
    }
#endif

    /* Initialize variables */
    long x = map2DChainGid2Id(tci->_azim, tci->_x);
    Track **chain_2D = _tracks_2D_chains[tci->_azim][x];

    // Parameters on the x-y plane
    double L = getCyclicLength(tci->_azim);

    // Parameters on the l-z plane, indexed by the first link
    int lz = tci->_lz;
    int nl = _num_l[tci->_azim][tci->_polar];
    int nz = _num_z[tci->_azim][tci->_polar];
    double dz = _dz_eff[tci->_azim][tci->_polar];
    ...
    // Find the 2D track the 3D starting point reside
    long links = getNum2DLinks(tci->_azim);
    double l_track = -1;
    double l_sum = 0;
    bool nudged = false;
    double l_nudge = TINY_MOVE * 10;
    for (link_index = 0; link_index < links; link_index++)
    {
      // Accumulate track length
      l_track = chain_2D[link_index]->getLength();
      l_sum += l_track;
      if (l_start <= l_sum)
      {
        if (l_sum - l_start < TINY_MOVE)
    ...
    if (l_start > l_sum)
    {
      log::ferror("The starting point of the 3D track is projected "
                  "outside the 2D track, l_start = %f > l_sum = %f",
                  l_start, l_sum);
    }
    ...
    double scale = (l_sum - l_start) / l_track;

    /* Assume P_start is the starting point
     * P = P_end + scale * (P_start - P_end) */
    x1 = p_end->getX() + scale * (p_start->getX() - p_end->getX());
    y1 = p_end->getY() + scale * (p_start->getY() - p_end->getY());

    /* If the start point was nudged, nudge it back */
    if (nudged)
    {
      x1 += l_nudge / l_track * (p_start->getX() - p_end->getX());
      y1 += l_nudge / l_track * (p_start->getY() - p_end->getY());
    }
                                        ...
                                        bool save_tracks)
  {
    ...
    // 该函数同时也计算了3D轨迹的起点(x1,y1,z1)和终点的纵坐标z2
    ...
    while (true)
    {
      ...
      /* Get the distance to the nearest X or Y boundary on x-y plane */
      double dx;
      double dy;
      double dl_xy;
 ...
 * exit. tci->_link appoints the desired track, -1 for the last link */
#ifdef USTB_
      end_of_chain = (fabs(dl_z - dl_xy) > TINY_MOVE && dl_z < dl_xy) || link >= getNum2DLinks(tci->_azim) - 1 || tci->_link == link - first_link;
#elif MIT_
      end_of_chain = dl_z < dl_xy || xy >= _num_y[tci->_azim] || tci->_link == link - first_link;
#endif

      // 初始化时，create_arrays是true；OTF时，这两个都是false
      if (create_arrays || save_tracks)
      {

        /* 这个数组统计了每个 z-stack 中的轨迹数量。
         * 也就是说，当2D平面上的一条轨迹确定了一个 z-stack 时，下面这几句统
         * 计出这个 z-stack 中3D轨迹的数量。xy 是在这个方法里递增的，对应着
         * 平面轨迹；lz 是在外层调用者里递增的，对应3D轨迹链。所以这里的算法
         * 是：遍历轨迹链，遍历链中的节，把节累计到相应的 stack 中。而不是遍
         * 历 stack。*/
        /* Get the index in the z-stack */
        int z = _tracks_per_stack[azim][xy][polar];
        _tracks_per_stack[azim][xy][polar]++; // 计入贡献
        ...
        // Save tracks for EXPLICIT ray tracing
        if (_segment_formation == +segmentationType::EXPLICIT_3D && save_tracks)
        {
      ...
      {
        // Move to the next 2D track
        track_2D = _tracks_2D_chains[tci->_azim][tci->_x][link];

        /* Move the starting x-coord to account for periodic BCs for chains
         * In the case of reflective BCs, the starting point is just the end
         * point of the previous link */
        if (!isReflectiveCyclic())
        {
          Point *next_start = track_2D->getStart();
          x2 = next_start->getX();
          y2 = next_start->getY();
        }
      }
    ...
    /* Set the start and end points of the last or desired link*/
    track->getStart()->setCoords(x1, y1, z1);
    track->getEnd()->setCoords(x2, y2, z2);

    /* Set the 3D link index, if it has not been set */
    if (tci->_link == -1)
   ...
   * @brief 为几何体中的每条挤出轨迹生成2D分段，
   *        同时初始化轴向挤出区域以及3D平源区(FSRs)。
   */
  void TrackGenerator3D::segmentizeExtruded()
  {

    log::finfo("Ray tracing for axially extruded track segmentation...");
    /* 用于轴向挤出轨迹分段的射线追踪... */
...
#ifdef ENABLE_MPI_ // MPI 预分段（仅在开启 ENABLE_MPI_ 且周期性轨迹被分解时）：
    long num_efsrs = 0;
    // mpi::isPrdTrackDecomposed() 为真说明周期轨迹被不同进程拆分，每个进程需要在正式分段前确定 FSR 的划分一致性。
    if (mpi::isPrdTrackDecomposed())
    {
      // pre-segmenting for FSR generation
      log::finfo("Pre-segmenting tracks to generate extruded FSRs...");
      /* 预分段轨迹以生成extrudedFSRs... */

      preSegmentizeExtruded(z_coords);

      // number of fsrs after pre-segmentation
      // 预分段后的FSR数量
      num_efsrs = _geometry->getNumExtrudedFSRs();
      log::verbose_once("Number of extruded FSRs created before segmenting = {}", num_efsrs);
    }
#endif

    log::info("Segmenting 2D tracks on the superposition plane...");
    /* 在重叠平面上对2D轨迹进行分段... */

    /* Loop over all extruded Tracks */
    long num_2D_tracks = getMyNum2DTracks();
    Progress progress(num_2D_tracks, "Segmenting 2D Tracks", 0.1, _geometry, true);
    // 创建进度跟踪器，用于显示2D轨迹分段的进度
...
#pragma omp parallel for schedule(dynamic)
    ...
    {
      progress.incrementCounter();                                    // 让进度条前进
      _geometry->segmentizeExtruded(_tracks_2D_array[uid], z_coords); // 涉及CMFD内容
    }

    _timer->stopTimer("Extruded Segmenting");

    // 3D segments are available now, so set the flag.
    _contains_2D_segments = true;

#ifdef ENABLE_MPI_ // MPI 校验
    // If periodic track desomposition is performed, check the number of extruded FSRs
    if (mpi::isPrdTrackDecomposed())
    {
      long tot_num_efsrs = _geometry->getNumExtrudedFSRs();
      if (num_efsrs < tot_num_efsrs)
      {
        log::error("Pre-segmentation failed, {} out of {}, please report "
                   "this to antmoc repository as a bug",
                   num_efsrs, tot_num_efsrs);
      }

      long global_efsrs;
      MPI_Allreduce(&tot_num_efsrs, &global_efsrs, 1, mpi::getDatatype<long>(), MPI_MAX,
                    mpi::getCommSharedDomain());
      if (tot_num_efsrs != global_efsrs)
      {
        log::error("The number of extruded FSRs is inconsistant across processes: "
                   "Local = {}, Global = {}",
                   tot_num_efsrs, global_efsrs);
      }
    }
    ...
    // Count the number of segments on each track and initialize buffers for segments
    // 计算每条轨迹上的分段数量并初始化分段缓冲区
    _timer->startTimer();
    countSegments();
    log::result("Number of local 3D segments = {}", _num_segments);

    // Report temporary segments storage
    // Segments may be counted again, so we just comment out this line and
    // delay the report to Solver.
    // reportTemporarySegmentsStorage();
    // 报告临时分段存储

#if defined(ENABLE_MPI_) && defined(ENABLE_PROFILE_)
    reportSegmentsWorkload();
    // 在MPI和性能分析模式下，报告分段工作负载的分布情况
#endif
    _timer->stopTimer("Segments Counting");
  }

  /**
   * @brief Generate segments for each Track across the Geometry.
   * @brief 为几何体中的每条轨迹生成分段。
   */
  void TrackGenerator3D::segmentize()
  {
    ...
    if (_segment_formation != +segmentationType::EXPLICIT_3D)
    {
      segmentizeExtruded(); // 使用OTF求解
      return;
    }

    log::finfo("Ray tracing for 3D track segmentation...");
    /* 3D轨迹分段的射线追踪... */

    long num_segments = 0;
    Progress progress(_num_3D_tracks, "Segmenting 3D Tracks", 0.1, _geometry,
                      true);
    // 创建进度跟踪器，用于显示3D轨迹分段的进度

    /* Loop over all Tracks */                                               // FIXME Move openmp section over all tracks
    /* 遍历每个轨迹堆,执行 3D 轨迹分段(segmentize3D),并初始化TSI 结构体。 */ // 待修复：将OpenMP部分移至所有轨迹上
    for (int a = 0; a < _num_azim / 2; a++)
    {
      // 遍历所有方位角（只需遍历一半，因为另一半是对称的）
#pragma omp parallel for schedule(dynamic)
      for (int i = 0; i < getMyNum2DTracks(a); i++)
      {
        int gi = map2DTrackId2Gid(a, i); // get the global id for later use
        for (int p = 0; p < _num_polar; p++)
        {
          for (int z = 0; z < _tracks_per_stack[a][gi][p]; z++)
          {
            // 遍历当前方位角和极角下的所有z方向轨迹
            progress.incrementCounter();
            // 遍历当前z-stack
...
#pragma omp atomic update
    ...
    float mem_mb = static_cast<float>(num_segments) * sizeof(segment) / (1 << 20);
    log::verbose("Explicit 3D segments storage = {:.3f} MiB", mem_mb);
    // 计算并输出3D分段存储所需的内存大小（以MiB为单位）
    // 这对于评估计算资源需求很有用
  }
                                                     ...
                                                     long track_id)
  {
    ...
    log::ferror("Unable to find a 3D track associated with the given track"
                "ID %ld during coordinate retrieval",
                track_id);
    return;
  }
  ...
  void TrackGenerator3D::create3DTracksArrays()
  {
    ...
    // Loop over azims of all tracks, not chains
    for (int a = 0; a < _num_azim / 2; a++)
    {
      tracks_per_azim = 0;
      for (long i = 0; i < getNum2DTracks(a); i++)
      {
        for (int p = 0; p < _num_polar; p++)
        {
    ...
    _my_num_3D_tracks = num_tracks;
    _num_3D_tracks = num_tracks;

#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    {
      MPI_Allreduce(&num_tracks, &_num_3D_tracks, 1, mpi::getDatatype<long>(), MPI_SUM,
                    mpi::getCommSharedDomain());
      log::profile("Average number of 3D Tracks per process = {}",
                   _num_3D_tracks / mpi::getNumProcsAuto());
      log::profile("Number of local 3D Tracks = {}", num_tracks);
    }
#endif
    log::result("Number of 3D Tracks in spatial domain = {}", _num_3D_tracks);

    // Count memory usage for Tracks
    double mem_usage = countTracksMemory();
    double tot_mem_usage = mem_usage;
#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    {
      MPI_Allreduce(&mem_usage, &tot_mem_usage, 1, mpi::getDatatype<double>(), MPI_SUM,
                    mpi::getCommSharedDomain());
      log::profile("Memory allocated for local Tracks = {:.3f} MiB", mem_usage);
    }
#endif
    log::info("Memory allocated for Tracks in spatial domain = {:.3f} MiB", tot_mem_usage);

    /* 如果使用EXPLICIT_3D方法，
     * 给tracks_3D分配内存，并把_tracks_per_stack中的元素全初始化成0 */
    /* Allocate tracks arrays if using explicit ray tracing */
    if (_segment_formation == +segmentationType::EXPLICIT_3D)
    {

      float mem_tracks_mb = static_cast<float>(num_tracks) * sizeof(Track3D) / (1 << 20);
      log::info("Explicit 3D Track storage = {:.3f} MiB", mem_tracks_mb);

      _tracks_3D = new Track3D ***[_num_azim / 2];
      for (int a = 0; a < _num_azim / 2; a++)
      {
        _tracks_3D[a] = new Track3D **[getMyNum2DTracks(a)];
        for (long i = 0; i < getMyNum2DTracks(a); i++)
        {
  ...
  {
    log::finfo("Initializing a default angular quadrature...");
    _quadrature = std::make_shared<GLPolarQuad>();
  ...
  std::string TrackGenerator3D::getTestFilename(std::string directory)
  {
    ...
    std::string quad_type = _quadrature->getQuadratureType()._to_string();
  ...
  void TrackGenerator3D::checkBoundaryConditions()
  {
    ...
    if ((_geometry->getMinZBoundaryType() == PERIODIC &&
        ...
        (_geometry->getMinZBoundaryType() != PERIODIC &&
         _geometry->getMaxZBoundaryType() == PERIODIC))
      log::ferror("Cannot create tracks with only one z boundary"
                  " set to PERIODIC");
  ...
  void TrackGenerator3D::allocateTemporaryTracks()
  {
    ...
    /* Report memory usage */
    double size_mb = static_cast<double>(_max_num_tracks_per_stack) * _num_threads * sizeof(Track3D) / (1 << 20);
    log::verbose("Temporary Track storage per domain = {:.3f} MiB", size_mb);

    /* Allocate new temporary tracks arrays */
    // 给每个线程分配两个vector，长度足够容纳任一stack中的轨迹
    for (int t = 0; t < _num_threads; t++)
    {
  ...
  void TrackGenerator3D::getTrackOTF(Track3D *track, TrackStackIndexes *tsi)
  {
    ...
    try
    {
      ...
      /* Set the start and end points 我们用的是OTF*/
      if (_segment_formation == +segmentationType::EXPLICIT_3D)
      {
        Track3D *track_3D =
      ...
      {
        /* Set starting and end points of track according to TCI
         * Compute the link index and save it to TCI */
      ...
      setLinkingTracks(tsi, &tci, true, track);  // 设置track的forward连接轨迹
      setLinkingTracks(tsi, &tci, false, track); // 设置track的backward连接轨迹
    ...
    {
      log::ferror("Unable to get track otf (%d, %d, %d, %d)."
                  " Backtrace:\n%s",
                  tsi->_azim, tsi->_xy, tsi->_polar,
                  tsi->_z, e.what());
    }
  }
  ...
  void TrackGenerator3D::convertTSItoTCI(TrackStackIndexes *tsi,
                                         TrackChainIndexes *tci)
  {
    // A reflective track chain has only one azimuthal angle.
    // In ANT-MOC, however, links of the chain may have angles
    // supplementary to each other.
    if (isReflectiveCyclic() && tsi->_azim >= _num_azim / 4)
      tci->_azim = _num_azim / 2 - tsi->_azim - 1;
    else
      tci->_azim = tsi->_azim;

    tci->_x = get2DChainIndex(tsi->_azim, tsi->_xy);
    tci->_polar = tsi->_polar;
    tci->_lz = _first_lz_of_stack[tsi->_azim][tsi->_xy][tsi->_polar] + tsi->_z;

    setTCILinkIndex(tci, tsi);
  }
  ...
  void TrackGenerator3D::convertTCItoTSI(TrackChainIndexes *tci,
                                         TrackStackIndexes *tsi)
  {
    int link = get2DLinkIndex(tci);
    tsi->_azim = get2DLinkAzimIndex(tci->_azim, link);
    tsi->_xy = _tracks_2D_chains[tci->_azim][tci->_x][link]->getXYIndex();
   ...
   */
  int TrackGenerator3D::get2DLinkIndex(TrackChainIndexes *tci)
  {
    return getFirst2DTrackLinkIndex(tci) + tci->_link;
  }
   ...
   */
  void TrackGenerator3D::setTCILinkIndex(TrackChainIndexes *tci,
                                         TrackStackIndexes *tsi)
  {
    int first_link = getFirst2DTrackLinkIndex(tci);
    Track *track_2D = &_tracks_2D[tsi->_azim][tsi->_xy];
  ...
  int TrackGenerator3D::getNum3DTrackChainLinks(TrackChainIndexes *tci)
  {
    ...
    int first_link = getFirst2DTrackLinkIndex(tci);
    int link = first_link;
    long xy = _tracks_2D_chains[tci->_azim][tci->_x][link]->getXYIndex(); // convert chain id to track id
    int azim;
    ...
    while (true)
    {
      azim = get2DLinkAzimIndex(tci->_azim, link);
      xy = _tracks_2D_chains[tci->_azim][tci->_x][link]->getXYIndex();

      min_lz = _first_lz_of_stack[azim][xy][polar];
      max_lz = _tracks_per_stack[azim][xy][polar] + min_lz - 1;

      if (polar < _num_polar / 2 && lz > max_lz)
        break; // 锐角，轨迹的lz小于等于当前z-stack中的最大lz
      else if (polar >= _num_polar / 2 && lz < min_lz)
        break; // 钝角，轨迹的lz大于等于当前z-stack中的最小lz

      link++;

// Util the last link
#ifdef USTB_
      if (link >= getNum2DLinks(azim))
        break;
#elif MIT_
      if (xy >= _num_y[azim])
        break;
  ...
  /// \details This method is a simple version for ANT-MOC
  /// \param tsi The stack indexes
  /// \param tci The chain indexes
  ...
  void TrackGenerator3D::setLinkingTracks(TrackStackIndexes *tsi,
                                          ...
                                          Track3D *track)
  {
    // Assume that the TSI and TCI are both mine
    Track *track_2D = &_tracks_2D[tsi->_azim][tsi->_xy];
    ...
    int nl = _num_l[tci->_azim][tci->_polar];
    int lz = tci->_lz;
    // int ac = _num_azim / 2 - tci->_azim - 1;  // 方位角补角的索引
    int pc = _num_polar - tci->_polar - 1; // 极角补角的索引
    // unused
    ...
    if (tci->_polar < _num_polar / 2)
    {
      ...
      if (tci->_link == num_links - 1 && lz >= nz && outgoing)
      {
        ...
        tci_prdc._lz = lz - nz;              // 满足周期边界条件的下一轨迹链的索引
        tci_refl._polar = pc;                // 满足全反射边界条件的下一轨迹链的极角
        tci_refl._lz = nl + 2 * nz - lz - 1; // 满足全反射边界条件的下一轨迹链的索引

        /* PERIODIC BC */
        if (!isAxialReflectiveCyclic())
          tci_next._lz = tci_prdc._lz;

        /* REFLECTIVE OR VACUUM BC */
        else
        {
          tci_next._polar = tci_refl._polar;
          tci_next._lz = tci_refl._lz;
        }

        /* Check for a double reflection */
        convertTCItoTSI(&tci_prdc, &tsi_prdc);

        // 满足周期边界条件的下一条轨迹和当前轨迹不在同一z-stack内
      ...
      /* SURFACE_Y_MIN (the leftmost surface in l-z plane) */
      else if (tci->_link == 0 && lz >= nl && !outgoing)
      {
        ...
        tci_refl._lz = tci_prdc._lz;
        tci_refl._link = tci_prdc._link;
      ...
      /* The last link reaching the rightmost boundary of the l-z plane
       * If periodic, it will reach SURFACE_Y_MAX
       * If reflective, it will reach SURFACE_Y_MIN */
      ...
      // SURFACE_X_MIN or SURFACE_X_MAX or or SURFACE_Y_MIN or SURFACE_Y_MAX
      // 不是第一节或最后一节，而是中间某一节
      else if (outgoing)
      {
        ...
        tci_refl._link = tci->_link + 1; // 反射的情况看图更容易理解
      }
    ...
    } // end of polars in (0, PI/2)
    else
    {
      // tci索引的轨迹链为钝角，指向z轴负方向
  ...
  void TrackGenerator3D::getTSIByIndex(long id, TrackStackIndexes *tsi)
  {
    ...
    int xy; // Local track index for ANT-MOC cyclic tracking
    for (xy = 0; xy < getMyNum2DTracks(a); xy++)
    {
      if (id < _cum_tracks_per_xy[a][xy])
      {
        tsi->_xy = map2DTrackId2Gid(a, xy);
        break;
      }
    }

    if (tsi->_xy == -1)
      log::ferror("could not generate TSI xy from track ID: %ld", id);

    for (int p = 0; p < _num_polar; p++)
    {
      if (id < _cum_tracks_per_stack[a][xy][p] +
                   _tracks_per_stack[a][xy][p])
      {
        tsi->_polar = p;
        tsi->_z = id - _cum_tracks_per_stack[a][xy][p];
        return;
      }
    }

    log::ferror("could not generate TSI from track ID: %ld", id);
  }
  ...
  void TrackGenerator3D::writeExtrudedFSRInfo(FILE *out)
  {
    ...
    int num_2D_tracks = getMyNum2DTracks();
  ...
  void TrackGenerator3D::readExtrudedFSRInfo(FILE *in)
  {
    ...
    log::fverbose("Status of reading extrudedFSR info: %d", ret);
  }
  ...
  /// \brief Get the total number of 3D chain stacks,
  /// \details A 3D chain stack can be indexed by azim, 2D chain
  ///          and polar. It is similar to track stacks.
  long TrackGenerator3D::getNum3DChainStacks()
  {
    long num_chains = 0;
    for (int a = 0; a < getNumChainAzims(); a++)
    {
      for (int x = 0; x < getNum2DChains(a); x++)
        num_chains += _num_polar;
    }
    return num_chains;
  }

  /// \brief Get my number of 3D chain stacks
  long TrackGenerator3D::getMyNum3DChainStacks()
  {
    long num_chains = 0;
    for (int a = 0; a < getNumChainAzims(); a++)
    {
  ...
  /// \brief Get the number of 3D chains
  long TrackGenerator3D::getNum3DChains()
  {
    long num_chains = 0;
    for (int a = 0; a < getNumChainAzims(); a++)
    {
      ...
      for (int p = 0; p < _num_polar; p++)
      {
        num_axial += _num_l[a][p] + _num_z[a][p];
      }
      num_chains += num_axial * getNum2DChains(a);
    }
    return num_chains;
  }

  /// \brief Get the number of my 3D chains
  long TrackGenerator3D::getMyNum3DChains()
  {
    long num_chains = 0;
    for (int a = 0; a < getNumChainAzims(); a++)
    {
      ...
      num_chains += num_axial * getMyNum2DChains(a);
    }
    return num_chains;
  }
  ...
  void TrackGenerator3D::preSegmentizeExtruded(const std::vector<double> &z_coords)
  {

    long num_2D_tracks = getMyNum2DTracks();
    Progress progress(num_2D_tracks, "Pre-segmenting 2D Tracks (superposition)",
                      0.1, _geometry, true);
...
#pragma omp parallel for schedule(dynamic)
    ...
    {
      progress.incrementCounter();
      _geometry->segmentizeExtruded(_tracks_2D_array[uid], z_coords, true);
    }

    _timer->stopTimer("Pre-segmenting");

    // Reduce all of the ExtrudedFSRs
    _timer->startTimer();
    _geometry->allReduceFSRs(z_coords, true);
    _timer->stopTimer("Reduce ExtrudedFSRs");
  }
#endif // ENABLE_MPI_
  ...
  void TrackGenerator3D::printTimerReport()
  {
    ...
    if (mpi::isPrdTrackDecomposed())
    {
      _timer->printSplit("Pre-segmenting", "ExtrudedFSRs creation",
                         2, segment_string);
      _timer->printSplit("Reduce ExtrudedFSRs", "ExtrudedFSRs reduction",
                         2, segment_string);
    }
#endif

    _timer->printSplit("Extruded Segmenting", "Segmenting on Superposition Plane",
                       2, segment_string);

    _timer->printSplit("Initialize Axial FSRs", "Axial FSRs Initialization",
                       2, segment_string);

    _timer->printSplit("Segments Counting", "Segments Counting",
                       2, segment_string);
...
#ifdef USTB_
  void TrackGenerator3D::printObjectStatistics()
  {

    auto part_length = log::get_line_length() / 3;

    long n1 = getNum2DChains();
    long n2 = getNum3DChains();
    long n3 = _geometry->getNumExtrudedFSRs();

    log::separator("-");

    log::result("{1: ^{0}}{2: ^{0}}{3: ^{0}}",
                part_length, "# 2D chains", "# 3D chains", "# ExtFSRs");
  ...
  /// \brief Count memory used by tracks
  /// \param format 'K', 'M' or 'G'
  double TrackGenerator3D::countTracksMemory(char format)
  {
    ...
    // by azim
    num = _num_azim / 2.0;
    size = sizeof(_tracks_2D[0]) + sizeof(_num_x[0]) + sizeof(_num_y[0]) + sizeof(_num_chains[0]) + sizeof(_cyclic_length[0]) + sizeof(_tracks_per_azim[0]) + sizeof(_tracks_2D_chains[0]) + sizeof(_dz_eff[0]) + sizeof(_dl_eff[0]) + sizeof(_num_z[0]) + sizeof(_num_l[0]) + sizeof(_cum_tracks_per_stack[0]) + sizeof(_cum_tracks_per_xy[0]) + sizeof(_tracks_per_stack[0]) + sizeof(_first_lz_of_stack[0]);
#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
      size += sizeof(_my_num_chains[0]);
#endif
    total_mem += num * size;
    // by xy
    num = (double)getNum2DTracks();
    size = sizeof(Track) + sizeof(_cum_tracks_per_stack[0][0]) + sizeof(_cum_tracks_per_xy[0][0]) + sizeof(_tracks_per_stack[0][0]) + sizeof(_first_lz_of_stack[0][0]);
    total_mem += num * size;

    num = (double)getMyNum2DTracks();
    size = sizeof(_tracks_2D[0][0]) + sizeof(_tracks_2D_array[0]);
    total_mem += num * size;

    num = (double)getMyNum2DChains();
    // we have loop over chains to count links precisely
    size = sizeof(_tracks_2D_chains[0][0]) + sizeof(_tracks_2D_chains[0][0][0]);
    total_mem += num * size;

    // by azim and polar angles
    num = _num_azim / 2.0 * _num_polar;
    size = sizeof(_dz_eff[0][0]) + sizeof(_dl_eff[0][0]) + sizeof(_num_z[0][0]) + sizeof(_num_l[0][0]);
    total_mem += num * size;

    // by tracks and polar angles
    num = (double)getMyNum2DTracks() * _num_polar;
    size = sizeof(_cum_tracks_per_stack[0][0][0]) + sizeof(_tracks_per_stack[0][0][0]) + sizeof(_first_lz_of_stack[0][0][0]);
    total_mem += num * size;

    // explicit 3D tracks
    if (_segment_formation == +segmentationType::EXPLICIT_3D)
      total_mem += _num_azim / 2.0 * sizeof(_tracks_3D[0]) + getMyNum2DTracks() * sizeof(_tracks_3D[0][0]) + getMyNum2DTracks() * _num_polar * sizeof(_tracks_3D[0][0][0]) + getMyNum3DTracks() * sizeof(Track3D);
    return total_mem / base;
  }

#ifdef ENABLE_MPI_
  void TrackGenerator3D::reportSegmentsWorkload()
  {
    double workload = static_cast<double>(getNumSegments());
    double tot_value;
    // Compute the propotion of workload
    MPI_Allreduce(&workload, &tot_value, 1, MPI_DOUBLE, MPI_SUM, mpi::getMPIComm());
    workload /= tot_value;

    // Compute the sample range
    double min_load, max_load;
    MPI_Reduce(&workload, &min_load, 1, MPI_DOUBLE, MPI_MIN, 0, mpi::getMPIComm());
    MPI_Reduce(&workload, &max_load, 1, MPI_DOUBLE, MPI_MAX, 0, mpi::getMPIComm());
    if (mpi::isMPIRoot())
    {
      log::finfo("Sample range of segments/total segments = %.4f%%", (max_load - min_load) * 100);
    }
  }
#endif

  void TrackGenerator3D::reportTemporarySegmentsStorage()
  {
    /* Determine storage size of temporary segments */
    float max_size = static_cast<float>(_num_seg_matrix_columns);
    float max_size_mb = max_size * _num_threads * sizeof(segment) / (1 << 20);

#ifdef ENABLE_MPI_
    if (mpi::isDomainDecomposed())
    {
      MPI_Allreduce(MPI_IN_PLACE, &max_size_mb, 1, mpi::getDatatype<float>(),
                    MPI_MAX, mpi::getMPIComm());
    }
#endif

    log::profile("Number of temporary segments = {}", _max_num_segments);

    log::verbose_once("Max temporary segment storage per domain = {:.3f} MiB", max_size_mb);
  }
  ...
  std::vector<TrackMeshData> TrackGenerator3D::getFormattedTracks()
  {

    std::vector<TrackMeshData> track_coords; ///< A vector of track mesh data
    track_coords.reserve(_my_num_3D_tracks);

    int num_2D_tracks = getMyNum2DTracks();
    Track **tracks_2D = get2DTracksArray();
    int ***tracks_per_stack = getTracksPerStack();
    // int num_azim = getNumAzim();
    int num_polar = getNumPolar();
    ...
    for (int ext_id = 0; ext_id < num_2D_tracks; ext_id++)
    {
      ...
      for (int p = 0; p < num_polar; p++)
      {
        ...
        for (int z = 0; z < tracks_per_stack[tsi._azim][tsi._xy][p]; z++)
        {
          ...
          tsi._polar = p;
          tsi._z = z;
          getTrackOTF(&track_3D, &tsi);

          // Push back an object of TrackMeshData
          track_coords.push_back(
              {get3DTrackID(&tsi),
               *track_3D.getStart(),
               *track_3D.getEnd()});
        }
