...
namespace antmoc
{
   ...
   */
  void Track::addSegment(const segment &segment)
  {
    ...
    {
      log::finfo("Unable to add a segment to Track. Backtrace:"
                 "\n%s",
                 e.what());
    }
  }
  ...
  void Track::removeSegment(int index)
  {
    ...
    {
      log::finfo("Unable to remove a segment from Track");
    }
  }
  ...
  void Track::insertSegment(int index, segment *segment)
  {
    ...
    {
      log::finfo("Unable to insert a segment into Track");
    }
  }
   ...
   *          its' periodic chain.
   *         This index is extended to link index of cyclic tracks
   * @param index The link index
   */
  void Track::setLinkIndex(long index)
  {
    _link_index = index;
  }

  /**
   * @brief Set a Track's chain index.
   */
  void Track::setChainIndex(long index)
  {
    _chain_index = index;
  }
   ...
   */
  long Track::getLinkIndex()
  {
    return _link_index;
  }

  long Track::getChainIndex()
  {
    return _chain_index;
  }
   ...
   *
   * 设置“下一条要追踪的轨迹 ID”；在周期情形它等于周期邻居，在反射情形等于反射邻居。
   * 沿 forward / backward 方向走完之后，接上去的那条 3D 轨迹的 ID。
   */
   ...
   */
  void Track::setXYIndex(long index)
  {
    _xy_index = index;
  }
   ...
   * 记录当沿 forward/backward 边界离开时，接续的那条轨迹要不要按它自己的 forward 方向追踪（true 表示保留 forward、false 表示要倒着追）。这只在反射模式需要，因为反射后方向可能翻。
   */
  ...
  void Track::setReversed(bool reversed)
  {
    _reversed = reversed;
  }

  bool Track::getReversed()
  {
    return _reversed;
  }
  ...
  std::string Track::toString()
  {
    ...
    string << "Track: start, x = " << _start.getX() << ", y = " << _start.getY() << ", end, x = " << _end.getX() << ", y = " << _end.getY() << ", phi = " << _phi;
    return string.str();
  }
