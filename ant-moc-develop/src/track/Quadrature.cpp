...
Quadrature::Quadrature() :
  _quad_type(quadratureType::EQUAL_ANGLE),
  _num_azim(0),
  _num_polar(0)
{ }
...
double Quadrature::getTheta(size_t azim, size_t polar) const {
  ...
  if (polar >= _num_polar)
    log::ferror("Attempted to retrieve theta for polar angle = "
               "%d and azim angle = %d when only %d polar angles are "
               "defined", polar, azim, _num_polar);
    ...
    log::ferror("Attempted to retrieve theta for polar angle = %d "
...
double Quadrature::getPhi(size_t azim) const {
  ...
  if (azim >= _num_azim)
    log::ferror("Attempted to retrieve theta for "
               "azim angle = %d when only %d azim angles are "
               "defined", azim, _num_azim);

  else if (_phis.size() == 0)
    log::ferror("Attempted to retrieve phi for "
...
double Quadrature::getAzimWeight(size_t azim) const {
  ...
  if (azim >= _num_azim)
    log::ferror("Attempted to retrieve the azimuthal weight for "
               "azimuthal angle = %d but only %d azimuthal angles "
               "are defined", azim, _num_azim);

  else if (_azim_weights.size() == 0)
    log::ferror("Attempted to retrieve weight for azimuthal angle = %d "
...
double Quadrature::getPolarWeight(size_t azim, size_t polar) const {
  ...
  if (polar >= _num_polar)
    log::ferror("Attempted to retrieve polar weight for polar angle = "
               "%d and azim angle = %d when only %d polar angles are "
               "defined", polar, azim, _num_polar);
  ...
  if (_polar_weights.size() == 0)
    log::ferror("Attempted to retrieve polar weight for polar angle = %d "
               "and azim angle = %d but the thetas have not been "
               "initialized", polar, azim);
...
double Quadrature::getWeight(size_t azim, size_t polar) const {
  ...
  if (polar >= _num_polar)
    log::ferror("Attempted to retrieve the weight for polar angle = "
    ...
    log::ferror("Attempted to retrieve weight for polar angle = %d "
...
/**
 * @brief Returns a reference to the Quadrature's vector of total weights.
 * @return a reference to the vector of total weights
 */
const std::vector<DoubleVec>& Quadrature::getTotalWeights() const {

  if (_total_weights.size() == 0)
    log::ferror("Attempted to retrieve the total weights vector "
...
const std::vector<DoubleVec>& Quadrature::getSinThetas() const {
  ...
  if (_sin_thetas.size() == 0)
    log::ferror("Attempted to retrieve the sin thetas vector "
               "but it has not been initialized");
...
const std::vector<DoubleVec>& Quadrature::getThetas() const {
  ...
  if (_thetas.size() == 0)
    log::ferror("Attempted to retrieve the thetas vector "
               "but it has not been initialized");
...
const DoubleVec& Quadrature::getPhis() const {
  ...
  if (_phis.size() == 0)
    log::ferror("Attempted to retrieve the phis vector "
               "but it has not been initialized");
...
const DoubleVec& Quadrature::getAzimWeights() const {
  ...
  if (_azim_weights.size() == 0)
    log::ferror("Attempted to retrieve the azimuthal weights vector "
               "but it has not been initialized");
...
const std::vector<DoubleVec>& Quadrature::getPolarWeights() const {
  ...
  if (_polar_weights.size() == 0)
    log::ferror("Attempted to retrieve the polar weights vector "
               "but it has not been initialized");
...
void Quadrature::setNumAzimAngles(size_t num_azim) {
  ...
  if (num_azim <= 0)
    log::ferror("Unable to set the number of azimuthal angles to %d "
...
void Quadrature::setNumPolarAngles(size_t num_polar) {
  ...
  if (num_polar <= 0)
    log::ferror("Unable to set the number of polar angles to %d "
...
void Quadrature::setThetas(const DoubleVec& thetas) {
  ...
  if (_num_polar/2 * _num_azim/4 != thetas.size())
    log::ferror("Unable to set %d thetas for Quadrature "
  ...
  for (size_t a=0; a < _num_azim/4; ++a) {
    for (size_t p=0; p < _num_polar/2; ++p) {
      if (thetas[ap] < 0. || thetas[ap] > M_PI_2)
        log::ferror("Unable to set theta to %f which is "
...
void Quadrature::setPolarWeights(const DoubleVec& weights) {
  ...
  if (_num_polar/2 * _num_azim/4 != weights.size())
    log::ferror("Unable to set %d polar weights for Quadrature "
               "with %d polar angles and %d azimuthal angles"
               " in each octant",
  ...
  for (size_t a=0; a < _num_azim/4; ++a) {
    for (size_t p=0; p < _num_polar/2; ++p) {
      if (weights[ap] < 0. || weights[ap] > M_PI_2)
        log::ferror("Unable to set polar weight to %f which is "
...
void Quadrature::setTheta(double theta, size_t azim, size_t polar) {
  ...
  if (theta <= 0.0 || theta >= M_PI_2)
    log::ferror("Unable to set theta for azim = %d and polar = %d "
...
void Quadrature::setPhi(double phi, size_t azim) {
  ...
  if (phi <= 0.0 || phi >= M_PI_2)
    log::ferror("Unable to set phi for azim = %d to %f which is not "
               "in the range (0.0, PI/2)", azim, phi);

  if (azim >= _num_azim/4)
    log::ferror("Unable to set phi for azim = %d since azim is not in"
...
void Quadrature::setAzimSpacing(double spacing, size_t azim) {
  ...
  if (spacing <= 0.0)
    log::ferror("Unable to set azimuthal spacing for azim = %d to %f "
                      "which is not strictly greater than zero", azim,
                      spacing);

  if (azim >= _num_azim/4)
    log::ferror("Unable to set azimuthal spacing for azim = %d since "
...
void Quadrature::setPolarSpacing(double spacing, size_t azim, size_t polar) {
  ...
  if (spacing <= 0)
    log::ferror("Unable to set polar spacing for azim = %d and polar = "
...
void Quadrature::setAzimWeight(double weight, size_t azim) {
  ...
  if (weight <= 0.0 || weight >= M_PI_2)
    log::ferror("Unable to set azim weight for azim = %d to %f which is "
               "not in the range (0.0, PI/2)", azim, weight);

  if (azim >= _num_azim/4)
    log::ferror("Unable to set azim weight for azim = %d since azim is "
...
void Quadrature::setPolarWeight(double weight, size_t azim, size_t polar) {
  ...
  if (weight <= 0.0 || weight >= M_PI_2) {
    log::ferror("Unable to set polar weight for azim = %d and "
               "polar = %d to %f which is not in the range (0.0, PI/2)",
               azim, polar, weight);
  }

  if (azim >= _num_azim/4) {
    log::ferror("Unable to set polar weight for azim = %d and polar = %d "
               "since azim is not in the range (0, _num_azim/4)", azim, polar);
  }
...
void Quadrature::initialize() {
  ...
  if (_num_polar == 0) {
    log::ferror("Unable to initialize Quadrature with zero polar angles. "
               "Set the number of polar angles before initialization.");
  }

  if (_num_azim == 0) {
    log::ferror("Unable to initialize Quadrature with zero azimuthal "
...
void Quadrature::precomputeWeights(bool solve_3D) {
  ...
  /* Check that track spacings have been set 检查轨迹间距和极角是否已设置*/
  if (_azim_spacings.size() == 0)
    log::ferror("Unable to precompute weights since track spacings have "
                      "not yet been set");
  ...
  if (_phis.size() == 0) {
    log::finfo("WARNING: Using uncorrected angles for weights");
  ...
  /* Compute the azimuthal weights 计算方位角权重*/
  double x1, x2;
  for (size_t a = 0; a < _num_azim/4; ++a) {
  ...
  /* Compute multiples of sine thetas and weights 计算 sin(theta) 和权重的乘积*/
  for (size_t a=0; a < _num_azim/4; ++a) {
    for (size_t p=0; p < _num_polar/2; ++p) {
...
TYPolarQuad::TYPolarQuad(): Quadrature() {
  _quad_type = +quadratureType::TABUCHI_YAMAMOTO;
  _num_polar = 6;
}
...
LeonardPolarQuad::LeonardPolarQuad(): Quadrature() {
  _quad_type = +quadratureType::LEONARD;
  _num_polar = 6;
}
...
GLPolarQuad::GLPolarQuad(): Quadrature() {
  _quad_type = +quadratureType::GAUSS_LEGENDRE;
  _num_polar = 6;
...
DoubleVec GLPolarQuad::getLegendreRoots(size_t n) {
  ...
  for (size_t iter=0; iter < MAX_LG_ITERS; ++iter) {
    ...
    if (all_roots_converged)
      ...
      log::ferror("Failed to converge Gauss-Legendre roots for %d roots",
                         n);
    }
...
EqualWeightPolarQuad::EqualWeightPolarQuad(): Quadrature() {
  _quad_type = +quadratureType::EQUAL_WEIGHT;
  _num_polar = 6;
}
...
/**
 * @brief Calculates total weights for every azimuthal/polar combination based
 *        on the equal angle polar quadrature.基于等角极正交计算每个方位角/极组合的总权重。
 * @param solve_3D Boolean indicating whether this is a 3D quadrature
 */
void EqualAnglePolarQuad::precomputeWeights(bool solve_3D) {
  ...
  /* Generate the sin thetas and weights using equations 420-422 of the
   * DOE Nucl. Eng. Handbook "Lattice Physics Computations" 使用Nucl的方程420-422生成正弦和权重。工程手册“晶格物理计算”*/
  size_t ap = 0;
  for (size_t a=0; a < _num_azim/4; ++a) {
    for (size_t p=0; p < _num_polar/2; ++p) {

      if (p < _num_polar/2 - 1)  //不是最后一个极角
        y1 = 0.5 * (cos(_thetas[a][p]) - cos(_thetas[a][p+1]));//当前极角 θ_p 和下一个极角 θ_(p+1) 之间的余弦值差的一半：
      else
        y1 = cos(_thetas[a][p]);   //当 p 是最后一个极角时，即 p == _num_polar / 2 - 1，y1 直接取当前极角的余弦值

      if (p >= 1)   //不是第一个极角
        y2 = 0.5 * (cos(_thetas[a][p-1]) - cos(_thetas[a][p])); //上一个极角 θ_(p-1) 和当前极角 θ_p 之间的余弦值差的一半：
      else
        y2 = 1.0 - cos(_thetas[a][p]);  //当 p 是第一个极角时，即 p == 0，y2 计算的是 1 减去当前极角的余弦值
