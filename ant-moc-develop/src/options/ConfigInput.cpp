...
namespace antmoc
{
  ...
  void ConfigInput::printArgumentsReport()
  {
    ...
    int num_procs_shared_domain = num_procs / domains[0] / domains[1] / domains[2];
  ...
  {
    return getOptionValueBool(_l_dump_settings) && !getOutputDirectory().empty();
  }
  ...
  bool ConfigInput::doesDumpFSRData()
  {
    ...
    return getOptionValueBool(_l_dump_fsrs) && !getOutputDirectory().empty() && !field_types.empty();
  }
  ...
  std::vector<int> ConfigInput::getNumModules()
  {
    ...
    auto s = getOptionValue(_l_modules); // 从配置文件中读取模块数量，返回 [x方向, y方向, z方向] 的模块数
    if (stringutils::isSpaces(s))
      s = "1, 1, 1";

    std::vector<int> modules;
    /*
    splitString()：字符串工具函数，按逗号分割字符串
    示例：输入 "3, 3, 10" → 输出 ["3", " 3", " 10"]
    */
    auto strvec = stringutils::splitString(s, ",");

    for (auto &w : strvec)
    {
      stringutils::trim(w);            // 去除字符串首尾的空格
      modules.push_back(std::stoi(w)); // std::stoi(w)：C++标准库函数，将字符串转换为整数（storing to integer）
    }
  ...
  size_t ConfigInput::getNumAzim()
  {
    ...
    if (num_azim % 4 != 0)
    {
      log::ferror("Unable to set # azimuthal angles to %d "
                  "since it is not a multiple of 4",
                  num_azim);
    }
  ...
  size_t ConfigInput::getNumPolar()
  {
    ...
    if (num_polar % 2 != 0)
    {
      log::ferror("Unable to set # polar angles to %d "
                  "since it is not a multiple of 2",
                  num_polar);
    }
