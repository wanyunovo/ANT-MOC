...
void Cmfd::setHexLatticeEnable(bool hexlattice_enable) {
  ...
  if (hexlattice_enable) { // Hexagonal
...
#pragma omp for
    for (int i = 0; i < _local_num_x * _local_num_y * _local_num_z; i++) {
      ...
      for (iter = _cell_fsrs.at(i).begin();
        ...
        /* Calculate total neutron production in the FSR 计算 FSR 中的总中子产生率*/
        double neutron_production = 0.0;  //每个FSR的中子产生
        for (int h = 0; h < _num_moc_groups; h++) {   //遍历该FSR的moc能群
...
CMFD_PRECISION Cmfd::getSurfaceDiffusionCoefficient(int cmfd_cell, int surface,
   ...
   * approipriate BC */
  if (global_cmfd_cell_next == -1) { //检查是否到达边界，即相邻单元是否不存在
  ...
  /* If surface is an interface or PERIODIC BC, use finite differencing 如果表面是界面或周期性边界条件，则使用有限差分*/
    ...
    if (cmfd_cell_next == -1) {  //如果 cmfd_cell_next == -1，说明相邻单元不在当前域中
    ...
    dif_surf = 2.0 * dif_coef * dif_coef_next
        / (delta * dif_coef + delta * dif_coef_next); //与B.23公式对应,其中delta为δrh的两倍,所以分子多乘了个2
    ...
    dif_surf_corr = -(sense * dif_surf * (flux_next - flux) + current)
        / (flux_next + flux);    //对应公式B.26,这个current好像多乘了个方向sense(不一定,有可能净流项的计算本身需要乘以sense方向)

    /* Flux limiting condition 通量限制条件*/
    if (_flux_limiting && moc_iteration > 0) {  //如果启用了通量限制 (_flux_limiting) 并且迭代次数大于零，检查修正系数与原始系数的比率
      double ratio = dif_surf_corr / dif_surf;
      if (std::abs(ratio) > 1.0) {  //如果比率大于 1.0，表示修正过大，需要限制,根据电流和通量重新计算 dif_surf 和 dif_surf_corr。
        //调整扩散系数
  ...
  if (CellinXYZBoundary(global_cmfd_cell, surface) || global_cmfd_cell_next == -1) {
    ...
    /* VACUUM BC */
      ...
      current_out = sense * _surface_currents->getValue
          (logical_cmfd_cell, surface*_num_cmfd_groups + group) / delta_interface;
      ...
      if (_old_dif_surf_valid) {
        CMFD_PRECISION old_dif_surf_corr = _old_dif_surf_corr->getValue
            (logical_cmfd_cell, surface*_num_cmfd_groups+group);
        dif_surf_corr = _relaxation_factor * dif_surf_corr +
            (1.0 - _relaxation_factor) * old_dif_surf_corr;
      }
    }
  ...
  /* If surface is an interface or PERIODIC BC, use finite differencing */
    ...
    if (cmfd_cell_next == -1) {
      ...
      if (!_empty_fsrs_cells[cmfd_cell_next]) {
        ...
        /* Get the inward current on the surface */
        current_in = _surface_currents->getValue
            (_logical_actual_map[cmfd_cell_next], surface_next*_num_cmfd_groups + group);
      }
    }
...
double Cmfd::computeKeff(int moc_iteration) {
  ...
  if (_hexlattice_enable)
    ...
    collapseXS();    //压缩截面数据 分割顶点和边流,存入surface_current ,计算CMFD网格和能群的chi,扩散系数，总，裂变，散射截面，体积以及old_flux
  ...
  //_M->printString();
  double k_eff;
  /* Solve the eigenvalue problem 求解特征值问题*/
  if (_hexlattice_enable) {
  ...
  if (fabs(k_eff + 1) < FLT_EPSILON && _num_cmfd_groups > _num_backup_groups) {
    ...
    copyCurrentsToBackup();  //当前的电流复制到备份求解器 
  ...
  if (reduced_group_solution) {  //如果使用少群求解器
...
#pragma omp for
    for (int i = 0; i < _local_num_x*_local_num_y*_local_num_z; i++) {
      ...
      for (int e = 0; e < _num_cmfd_groups; e++) {
        ...
        for (int g = 0; g < _num_cmfd_groups; g++) {  //这表示从 g 能群向当前 e 能群的散射贡献
        ...
        for (int s = 0; s < NUM_FACES; s++) {
          ...
          if (getCellNext(i, s, false, false) != -1) {  //检查当前表面 s 是否有邻居单元格
        ...
        /* Fission source term 裂变源项*/
        for (int g = 0; g < _num_cmfd_groups; g++) {
          value = material->getChiByGroup(e+1)
              * material->getNuSigmaFByGroup(g+1) * volume;  //裂变能谱*裂变截面*平均裂变中子数*体积 B.28的右侧项
...
#pragma omp parallel for
  for (int i = 0; i < _local_num_x * _local_num_y * _local_num_z; i++) {  //遍历CMFD网格
...
void Cmfd::initializeCurrents() {
  ...
  if (_balance_sigma_t) {  //在每次扫描计算时，根据 MOC 的解重新平衡和调整总截面（sigma-t）的值，以确保计算的一致性和准确性*/
...
void Cmfd::initializeCellMap() {
  ...
  for (int z = 0; z < _local_num_z; z++) {  //_cell_fsrs是一个二维数组，第一维对应一个CMFD，第二维对应该CMFD中所包含的FSR数组
    for (int y = 0; y < _local_num_y; y++) {  //一维index = z * (_local_num_y * _local_num_x) + y * _local_num_x + x
      for (int x = 0; x < _local_num_x; x++)
...
void Cmfd::allocateTallies() {
  ...
  for (int t=0; t < 3; t++) {  //t = 0 代表是扩散统计（计数器），1代表反应统计（计数器），2代表体积统计（计数器）这里的tally翻译为计数器或统计数值
    all_tallies[t] = new CMFD_PRECISION*[local_num_cells];
    for (int i=0; i < local_num_cells; i++) {  //i为CMFD网格数量的下标
...
void Cmfd::addFSRToCell(int cmfd_cell, long fsr_id) {
  if (_hexlattice_enable) { // 0000000000000
...
#pragma omp for
    for (int i=0; i < _local_num_x * _local_num_y * _local_num_z; i++) {
      ...
      for (int v = NUM_FACES + NUM_EDGES; v < NUM_SURFACES; v++) {  //划分的是所有的顶点
        ...
        for (int g=0; g < ncg; g++) {
          ...
          * which propagate through 3 edges 将顶点电流除以3，因为我们将分裂为3个surface，这些surface通过3条边传播到下一个cell粗网格*/
          ...
          for (iter = surfaces.begin(); iter != surfaces.end(); ++iter) {//循环6次
            ...
            if (local_cell != -1) {
              if (surface < NUM_FACES) {  //其中有三个值是面的
                _surface_currents->incrementValue(local_cell,
                                                  surface * ncg + g, current);
              }
              else { //添加边的电流(其余三个是边的,也是下一个CMFD网格的电流)
...
#pragma omp for
    for (int i=0; i < _local_num_x * _local_num_y * _local_num_z; i++) {
      ...
      for (int e = NUM_FACES; e < NUM_FACES + NUM_EDGES; e++) {  //划分边
        ...
        for (int g=0; g < ncg; g++) {
          ...
          * which propagate through 2 surfaces 将边电流除以2，因为我们将分裂为2个表面，这些表面通过2个表面传播*/
          ...
          /* Increment current for faces and edges adjacent to this vertex */
          for (iter = surfaces.begin(); iter != surfaces.end(); ++iter) {//循环4次
            cell = (*iter) / ns;
            surface = (*iter) % ns;

            /* Look for the CMFD cell on-domain */
            int local_cell = getLocalCMFDCell(cell);
            if (local_cell != -1) {  //2次加当前网格,2次加方向对应的下一个网格,他们的local_cell不一样
...
void Cmfd::getVertexSplitSurfaces(int cell, int vertex,
  ...
  for (int i=0; i < 3; i++) {  //遍历三个维度x,y,z
    int remainder_direction[3];
    int partial_direction[3];
    
    for (int j=0; j < 3; j++) {//遍历三个维度x,y,z,给remainder_direction和partial_direction赋值
      if (i == j) {  //对应上就把direction相应的值传给partial_direction,其余为0
        remainder_direction[j] = 0;//{0}
        partial_direction[j] = direction[j];  //{1}
      }
      else {    //对应不上就把direction相应的值传给remainder_direction,其余为0
  ...
  for (int i=0; i < 3; i++) {
    ...
    if ((cell_indexes[i] == 0 && direction[i] == -1) ||
      ...
      if (_boundaries[partial_surface] == REFLECTIVE) {   //处于边界情况
        surfaces->push_back(cell * ns + remainder_surface);//当前部分表面是反射边界，则将电流反射回当前单元
      }
      else if (_boundaries[partial_surface] == PERIODIC) {//当前部分表面是周期性边界，则将电流传播到下一个单元的相应位置
...
bool Cmfd::CellinXYZBoundary(int x, int y, int z, int surface) {
  ...
  else if (x == length && y <= mid) { //右边界,包含上下两端点
...
void Cmfd::getEdgeSplitSurfaces(int cell, int edge,
  ...
  for (int i=0; i < 3; i++) {//比如边为{1,1,0},则被拆分为partial_direction[0]={1,0,0},partial_direction[0]={0,1,0}
    if (direction[i] != 0) {
...
int Cmfd::getCellNext(int cell, int surface_id, bool global, bool neighbor) {
  ...
  if (global || _domain_communicator == NULL) {//如果没有域分解
...
void Cmfd::generateKNearestStencils() {
  ...
  if (_use_axial_interpolation && _local_num_z >= 3) {//选择插值器来保持三个节点中的每一个节点(当前单元和两个轴向邻居)中的平均通量,Z轴向CMFD的数量不少于3
      ...
      if (!_empty_fsrs_cells[global_ind]) {
        ...
        for (fsr_iter = _cell_fsrs.at(i).begin();
          ...
          for (int j=0; j < num_cells_in_stencil; j++){
            _k_nearest_stencils[fsr_id]
              .push_back(std::make_pair<int, double>
                        (int(j), getDistanceToCentroidHex(centroid, global_ind, j)));
          }
...
CMFD_PRECISION Cmfd::getUpdateRatio(int cell_id, int group, int fsr) {
  ...
  if (_centroid_update_on) {
    ...
    for (iter = _k_nearest_stencils[fsr].begin();
      ...
      if (iter->first != 4) {  //对于每个质心（不包括中心单元，即 iter->first != 4），计算与这些质心相关的 CMFD 单元的 ID
...
double Cmfd::getDistanceToCentroid(Point* centroid, int cell_id,
  ...
  if (x > 0 && y > 0 && stencil_index == 0) {   //x > 0 && y > 0  保证存在左下的这个CMFD 以下同
...
#pragma omp parallel for schedule(guided)
    ...
    if (_domain_communicator != NULL) {
      ...
      for (int y=0; y < _local_num_y; y++) {
        for (int z=0; z < _local_num_z; z++) {
          if (x_start - 1 >= 0) { //用于检查当前域左侧是否有相邻的域单元。如果 x_start - 1 >= 0，说明在当前域的左侧还有域单元存在，左侧边界不是整个几何的边界。
            global_ind = ((z_start + z) * _num_y + y + y_start) *
                            _num_x + x_start - 1;   //左侧域CMFD单元的全局索引
            /*
          ...
          if (x_end < _num_x) {  //用于检查当前域右侧是否到达几何边界的右侧
            global_ind = ((z_start + z) * _num_y + y + y_start) *
                            _num_x + x_end;
      ...
      for (int y=0; y < _local_num_y; y++) {
        for (int z=0; z < _local_num_z; z++) {
          if (x_start - 1 >= 0) {
            global_ind = ((z_start + z) * _num_y + y + y_start) *
                            _num_x + x_start - 1;
            _boundary_index_map.at(HEX_SURFACE_BETA_MIN)[global_ind] = z * _local_num_y
                                                              + y;
          }
          if (x_end < _num_x) {
            global_ind = ((z_start + z) * _num_y + y + y_start) *
            ...
            _boundary_index_map.at(HEX_SURFACE_BETA_MAX)[global_ind] = z * _local_num_y
                                                                + y;
          }
        }
      }
      ...
      for (int x=0; x < _local_num_x; x++) {
        for (int z=0; z < _local_num_z; z++) {
          if (y_start - 1 >= 0) {
            global_ind = ((z_start + z) * _num_y + y_start-1) *
                            _num_x + x + x_start;
            _boundary_index_map.at(HEX_SURFACE_DELTA_MIN)[global_ind] = z * _local_num_x
                                                                + x;
          }
          if (y_end < _num_y) {
            global_ind = ((z_start + z) * _num_y + y_end)
                          * _num_x + x + x_start;
            _boundary_index_map.at(HEX_SURFACE_DELTA_MAX)[global_ind] = z * _local_num_x
                                                                + x;
          }
        }
      }
      ...
      for (int x=0; x < _local_num_x; x++) {
        for (int y=0; y < _local_num_y; y++) {
          if (z_start - 1 >= 0) {
            global_ind = ((z_start-1) * _num_y + y + y_start) *
                          _num_x + x + x_start;
            _boundary_index_map.at(HEX_SURFACE_Z_MIN)[global_ind] = y * _local_num_x
                                                                + x;
          }
          if (z_end < _num_z) {
            global_ind = ((z_end) * _num_y + y + y_start) *
                            _num_x + x + x_start;
            _boundary_index_map.at(HEX_SURFACE_Z_MAX)[global_ind] = y * _local_num_x
                                                                + x;
          }
        }
      }
...
void Cmfd::initializeLattice(Point* offset) {
  ...
  if(_hexlattice_enable) {//Create HexLattice
  ...
  else {//Create RecLattice
    if (_non_uniform) {   //这个均匀不均匀体现在每个CMFD网格的宽度（包括XYZ）是不是由几何的总宽度除以各个方向上的的数量，比如一个方向总宽为6，方向上有3个CMFD网格，均匀的就是一个网格宽2了，不均匀的可能是宽度为2,1,3这种
...
void Cmfd::initializeBackupCmfdSolver() {
  ...
  if (_backup_group_structure.size() == 0) {  //如果没有指定备份群结构，则自动将所有群设置为一群
...
void Cmfd::ghostCellExchange() {
  ...
  for (int coord=0; coord < 3; coord++) {//遍历三个维度
    for (int d=0; d<2; d++) {  //最大面和最小面两个方向
...
void Cmfd::communicateSplits(bool faces) {
  ...
  计算序列化缓冲区的大小。
  */
  for (int coord=0; coord < 3; coord++) {//遍历 X, Y, Z 三个方向的坐标
    for (int d=0; d<2; d++) {  //遍历每个方向的两个面（最小面和最大面）
      ...
      for (int x=0; x < _local_num_x; x++) {
        ...
        for (int s=0; s < NUM_FACES; s++) {
          if (found_surfaces[s]) {
            ...
            if (faces) {  //若 faces 为 true，解包面电流数据并将非零值累加到 cell_id 对应的 _surface_currents 中
            ...
            else {//若 faces 为 false，则解包边缘电流数据
...
int Cmfd::convertDirectionToSurface(int* direction) {
  ...
  if (num_crossings == 1) {  //即方向向量中只有一个非零值，其余两个值为0,也就是一个面
    for (int i=0; i < 3; i++) {
  ...
  else if (num_crossings == 2) {//处理边的情况,边只有1个为0,其余两个非零
    surface += NUM_FACES;//首先+6,是因为边最小值就是6,从6开始的
    int ind1 = 0;
    int ind2 = 0; //ind1,ind2确定该边是由哪两个方向的面交叉而来的
    if (direction[0] == 0) {   //如果x方向为零,说明是y和z方向交叉的
      ind1 = direction[1];
      ind2 = direction[2];
      surface += 8;//因为涉及y和z的边是从14开始的,所以要先+8;
    }
    else if (direction[1] == 0) {//如果y方向为零,说明是x和z方向交叉的
      ind1 = direction[0];
      ind2 = direction[2];
  ...
  else if (num_crossings == 3) {//处理顶点的情况,三个全非零
...
void Cmfd::convertSurfaceToDirection(int surface, int* direction) {
  ...
  if (surface < NUM_FACES) {
    ...
    int n = 0;
    for (int i=0; i < 3; i++) {  //比如位于XY最小边 SURFACE_X_MIN_Y_MIN 6 第一行减去6,surface=0;
      if (i != skipped) {  //在三个轴中，跳过 skipped 轴，其他两个轴的方向由 ind 数组决定
        direction[i] = 2 * ind[n] - 1;  //x轴方向direction[0] = -1,位于负方向,y轴方向direction[0] = -1 也位于负方向,Z轴跳过
        n++;
      }
    }
  }
  else if (surface < NUM_SURFACES) {  //表面ID在NUM_FACES + NUM_EDGES和NUM_SURFACES之间（即它是一个顶点），计算该顶点对应的方向
 ...
 * 用于在 CMFD网格单元表面上累积当前段（segment）的电流贡献。
 * 这个函数通过处理段的流出角通量，计算并累积在每个网格单元表面的电流
...
void Cmfd::tallyCurrent(segment* curr_segment, float* track_flux,
  ...
  if (curr_segment->_cmfd_surface_fwd != -1 && fwd) {  //如果当前轨迹处于CMFD网格surface上且方向为正向
    surf_id = curr_segment->_cmfd_surface_fwd % NUM_SURFACES;
    cell_id = curr_segment->_cmfd_surface_fwd / NUM_SURFACES;
    tally_current = true;
  }
  else if (curr_segment->_cmfd_surface_bwd != -1 && !fwd) {//如果当前轨迹处于CMFD网格surface上且方向为反向
  ...
  if (tally_current) { //如果tally_current仍为false,则不需要统计中子流因为该线段不处于CMFD的surface上
    ...
    if (_SOLVE_3D) {  //三维求解
      ...
      if (surf_id < NUM_FACES) { //更新表面电流
        _surface_currents->incrementValues
            (local_cell_id, surf_id*ncg, (surf_id+1)*ncg - 1, currents);//将结果存放在_array数组中
      }
      else { //更新边和角的电流
        ...
        for (int g=0; g < ncg; g++)
          _edge_corner_currents[first_ind+g] += currents[g];  // 累积当前段的电流贡献到 _edge_corner_currents
      ...
      for (int e=0; e < _num_moc_groups; e++) {
        ...
        for (int p = 0; p < _num_polar/2; p++) {   //如果是二维,则把极角和能群的角通量全部累加起来
    ...
    if (!_empty_fsrs_cells[local_cell_id]) {
      ...
      if (_SOLVE_3D) {
        ...
        if (surf_id < HEX_NUM_FACES) {
          _surface_currents->incrementValues
              (logical_cell_id, surf_id*ncg, (surf_id+1)*ncg - 1, currents);
        }
        else {
  ...
  for (int i=0; i < _local_num_x * _local_num_y * _local_num_z; i++) {
    ...
    if (_hexlattice_enable) {
      ...
      for (int s=0; s < HEX_NUM_SURFACES; s++) {
        ...
        cmfd_cell_next = cell_next_ind[0] + cell_next_ind[1] * _local_num_x
                      + cell_next_ind[2] * (_local_num_x * _local_num_y);  //域一维索引
...
void Cmfd::GetXYSurfaces(int *cmfd_surfaces) {
  if(GetHexLatticeEnable()){   00000000000000000000
    for (int d=0; d<2; d++) {
    ...
    for (int d=0; d<2; d++) {
      ...
      if (local_surface == 2 || local_surface == 5) {   //2和5都是这个点只在Z轴的表面，不关联x和y轴
          cmfd_surfaces[d] = -1;
      }
      else if (local_surface > 9) {  //大于9说明这个点一定有和Z轴相关的边和角（因为如果只有XY轴，只有4边，4点，2个上述已经排除的）
        int cell = cmfd_surfaces[d] / NUM_SURFACES; //计算出CMFD单元格索引
        int half_surf = local_surface / 2;
        if (local_surface > 17) {    //说明有和Z轴相关的顶角  可能为：18,19,20,21,22,23,24,25（8个）
          int quart_surf = half_surf / 2;
        ...
        else {       //说明有和Z轴相关的边，可能为：10,11,12,13,14,15,16,17（8个）
