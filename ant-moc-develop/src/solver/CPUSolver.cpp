...
namespace antmoc
{
   ...
   */
  CPUSolver::CPUSolver(TrackGeneratorPtr track_generator)
      : CPUSolver(track_generator.get())
  {
  ...
  void CPUSolver::getFluxes(FP_PRECISION *out_fluxes, int num_fluxes)
  {
    ...
    if (num_fluxes != _NUM_GROUPS * _geometry->getNumTotalFSRs())
      log::ferror("Unable to get FSR scalar fluxes since there are "
                  "%d groups and %d FSRs which does not match the requested "
                  "%d flux values",
                  _NUM_GROUPS, _geometry->getNumTotalFSRs(),
                  num_fluxes);

    else if (_scalar_flux == NULL)
      log::ferror("Unable to get FSR scalar fluxes since they "
...
#pragma omp parallel for schedule(static)
  ...
  {
    if (num_threads <= 0)
      log::ferror("Unable to set the number of threads to %d "
                  "since it is less than or equal to 0",
                  num_threads);
    ...
    if (num_threads > 1 && provided < MPI_THREAD_SERIALIZED)
      log::fwarn("Not enough thread support level in the MPI library, "
    ...
    {
      auto segment_formation = _track_generator->getSegmentFormation();
      if ((segment_formation == +segmentationType::OTF_STACKS ||
           segment_formation == +segmentationType::OTF_TRACKS) &&
          _track_generator->getNumThreads() != num_threads)
      {

        log::warn("The number of threads used in track generation ({}) "
                  "should match the number of threads used in the solver ({}) "
  ...
  void CPUSolver::setFluxes(FP_PRECISION *in_fluxes, int num_fluxes)
  {
    if (num_fluxes != _NUM_GROUPS * _num_FSRs)
      log::ferror("Unable to set an array with %d flux values for %d "
                  " groups and %d FSRs",
                  num_fluxes, _NUM_GROUPS, _num_FSRs);
                                      ...
                                      FP_PRECISION source)
  {
    ...
    if (fabs(_fixed_sources(fsr_id, group - 1)) > FLT_EPSILON)
      log::fwarn("Overriding fixed source %f in FSR ID=%d group %g, with"
                 " %f",
                 _fixed_sources(fsr_id, group - 1), fsr_id, group, source);
  ...
  void CPUSolver::resetFixedSources()
  {
    ...
    for (auto &p : _fix_src_FSR_map)
      p.second = 0;

    /* Reset fixed source by cell map */
    for (auto &p : _fix_src_cell_map)
      p.second = 0;

    /* Reset fixed source by material map */
    for (auto &p : _fix_src_material_map)
      p.second = 0;
  ...
  void CPUSolver::initializeFSRs()
  {
    ...
    if (strcmp(_source_type.c_str(), "Flat") == 0)
      log::ferror("OpenMOC was compiled for linear sources only. Remove "
  ...
  void CPUSolver::initializeFluxArrays()
  {
    ...
    try
    {
      ...
      max_size_mb /= 2;
#endif
      log::info("Max boundary angular flux storage per domain = {:.3f} MiB", max_size_mb);
      ...
      max_size_mb = static_cast<double>(num_flux_arrays) * max_size * sizeof(FP_PRECISION) / (1 << 20);

      log::info("Max scalar flux storage per domain = {:.3f} MiB", max_size_mb);
    ...
    {
      log::error("Could not allocate memory for the fluxes");
    }
  }
  ...
  void CPUSolver::initializeFixedSources()
  {
         ...
         fsr_iter != _fix_src_FSR_map.end(); ++fsr_iter)
    {
      ...
      if (group <= 0 || group > _NUM_GROUPS)
        log::ferror("Unable to use fixed source for group %d in "
                    "a %d energy group problem",
                    group, _NUM_GROUPS);
...
#pragma omp parallel for schedule(static)
    for (long t = 0; t < _tot_num_tracks; t++)
    {
      ...
      {
        log::ferror("Starting currents not implemented yet for 2D MOC");
      }
    }
  }
  ...
  void CPUSolver::setupMPIBuffers()
  {
    ...
    if (mpi::isSpatialDecomposed())
    {
      ...
      log::finfo("Setting up MPI Buffers for angular flux exchange...");
      ...
      float size = 2 * message_length * num_domains * sizeof(float);
      float max_size;
      MPI_Allreduce(&size, &max_size, 1, MPI_FLOAT, MPI_MAX,
                    _geometry->getMPICart());
      log::verbose_once("Max track fluxes transfer buffer storage = {:.3f} MiB",
                        max_size / (1 << 20));
      ...
      log::info("Initializing Track connections across domains...");
...
#pragma omp critical
      ...
      log::info("Finished setting up MPI buffers...");
  ...
  {
    for (auto &row : _send_buffers)
      row.clear();
    _send_buffers.clear();

    for (auto &row : _receive_buffers)
      row.clear();
    _receive_buffers.clear();
    _neighbor_domains.clear();

    for (auto &row : _boundary_tracks)
      row.clear();
  ...
  void CPUSolver::resetBoundaryFluxes()
  {
    ...
    if (mpi::isSpatialDecomposed())
    {
      log::fverbose("Setting %ld incoming vaccum boundary fluxes to 0.",
...
#pragma omp parallel for
  ...
  void CPUSolver::printCycle(long track_start, int domain_start, int length)
  {
    ...
    for (int t = 0; t < length; t++)
    {
      ...
      if (rank == curr_rank)
      {
        ...
        /* Write information */
        log::fnode("Rank %d: Track (%f, %f, %f) -> (%f, %f, %f)", rank,
   ...
   * @param packing_indexes index of last track sent for each neighbor domain
   */
    ...
    while (true)
    {
      ...
      for (int i = 0; i < num_domains; i++)
      {
        ...
        if (first_track != -1)
        {
          ...
          if (!_MPI_sends[i])
          {
            MPI_Isend(&_send_buffers.at(i)[0], _track_message_size * _send_size.at(i), MPI_FLOAT, domain, 1, MPI_cart,
                      &_MPI_requests[i * 2]);
            _MPI_sends[i] = true;
          }
          else if (!_MPI_sends[i])
            _MPI_requests[i * 2] = MPI_REQUEST_NULL;

          /* Receive incoming flux */
          if (!_MPI_receives[i])
          {
            MPI_Irecv(&_receive_buffers.at(i)[0], _track_message_size * TRACKS_PER_BUFFER, MPI_FLOAT, domain, 1, MPI_cart,
                      &_MPI_requests[i * 2 + 1]);
            _MPI_receives[i] = true;
          }
          else if (!_MPI_receives[i])
      ...
      {
        _timer->stopTimer("Communication time");
        break;
      }
      ...
      for (int i = 0; i < num_domains; i++)
      {
        ...
        if (_MPI_receives[i])
        {
          ...
          for (int t = 0; t < TRACKS_PER_BUFFER; t++)
          {
            ...
            /* Get the Track ID */
            float *curr_track_buffer = &_receive_buffers.at(i)[t * _track_message_size];
            long *track_idx =
      ...
      _timer->stopTimer("Unpacking time");
    }

    /* Join MPI at the end of communication */
    MPI_Barrier(MPI_cart);
    _timer->stopTimer("Total transfer time");

    log::profile_once("Transferred scalar fluxes {} rounds per process", round_counter + 1);
    log::profile("Transferred scalar fluxes to {} neighbors per round", _neighbor_domains.size());
  }
#else
        ...
        for (int i = 0; i < num_domains; i++)
        {
          ...
          if (active_communication)
          {
            ...
            if (_receive_size.at(i) > 0 && !_MPI_receives[i])
            {
              ...
              MPI_Irecv(&_receive_buffers.at(i)[0], _track_message_size * _receive_size.at(i), MPI_FLOAT, domain, 1, MPI_cart,
                        &_MPI_requests[i * 2 + 1]);
              _MPI_receives[i] = true;
            }
            else if (!_MPI_receives[i])
              ...
              if (send_domain >= 0)
              {
                    ...
                    _boundary_tracks.at(i_next).size())
                {
                                            ...
                                            packing_indexes.at(i_next)))
                  {
                    ...
                    {
                      log_warn("MPI angular flux communication buffer"
                               " from rank {} to {} overflowed. Buffer memory "
  ...
  void CPUSolver::boundaryFluxChecker()
  {
    ...
    while (tester < num_ranks)
    {
      ...
      if (tester == my_rank)
      {
        ...
        log::fnode("Checking boundary fluxes for process %d", tester);
        ...
        for (long t = 0; t < _tot_num_tracks; t++)
        {
          ...
          for (int dir = 0; dir < 2; dir++)
          {
            ...
            if (bc == INTERFACE)
            {
              ...
              if (dest == -1)
                log::ferror("Track %d on domain %d has been found to have "
                            "an INTERFACE boundary but no connecting domain",
                            t,
                            my_rank);

              /* Send a request for info */
              MPI_Send(connection, 2, mpi::getDatatype<long>(), dest, 0, MPI_cart);
              ...
              if (fabs(point->getX() - x) > 1e-5 ||
                ...
                log::ferror("Track linking error: Track %d in domain %d "
              ...
              for (int pe = 0; pe < _fluxes_per_track; pe++)
              {
                if (fabs(angular_fluxes[pe] - _boundary_flux(t, dir, pe)) > 1e-7)
                {
                  ...
                  if (dir == 0)
                    ...
                    dir_string = "BWD";
                  log::ferror("Angular flux mismatch found on Track %d "
            ...
            else
            {
              ...
              if (bc == VACUUM)
                memset(&_boundary_flux(t, dir, 0), 0, sizeof(float) * _fluxes_per_track);

              /* Check angular fluxes */
              for (int pe = 0; pe < _fluxes_per_track; pe++)
              {
                if (fabs(_start_flux(connecting_idx, !connect_fwd, pe) - _boundary_flux(t, dir, pe)) > 1e-7)
                {
                  std::string dir_string;
                  ...
                  log::ferror("Angular flux mismatch found on Track %d "
                              ...
                              " angular flux at this location is %f but the "
                              "starting flux at connecting Track %d in domain %d "
                              "in the %s direction is %f",
                              t, my_rank, dir_string.c_str(),
              ...
              if (bc == REFLECTIVE)
              {
                ...
                if (fabs(point->getX() - x) > 1e-5 ||
                    ...
                    fabs(point->getZ() - z) > 1e-5)
                {
                  log::ferror("Track linking error: Reflective Track %d "
              ...
              if (bc == PERIODIC)
              {
                ...
                if (fabs(phi - track.getPhi()) < 1e-5 ||
                    fabs(theta - track.getTheta()) < 1e-5)
                  log::ferror("Periodic boundary found on Track %d "
                              "with azimuthal angle %f and polar angle %f but "
                              "the periodic Track at index %d has azimuthal "
                              " angle %f and polar angle %f",
                              t, track.getPhi(),
        ...
        {
          MPI_Send(broadcast, 2, mpi::getDatatype<long>(), i, 0, MPI_cart);
        }
      ...
      else
      {
        ...
        if (message)
        {
          ...
          long connection[2];
          MPI_Recv(connection, 2, mpi::getDatatype<long>(), tester, 0, MPI_cart, &stat);
          ...
          else
          {
            ...
            if (bc_transfer != INTERFACE)
              log::fnode("Node %d requested track %ld that doesn't end at"
                         "an interface boundary condition.",
                         tester, t);
            if (dest_transfer != tester)
              log::fnode("Node %d requested track %ld that connects with "
                         "domain %d.",
                         tester, t, dest_transfer);
    ...
    MPI_Barrier(MPI_cart);
    log::finfo("Passed boundary flux check");
  }
#endif // ENABLE_MPI_
...
#pragma omp parallel for schedule(static)
  ...
  void CPUSolver::flattenFSRFluxesChiSpectrum()
  {
    if (_chi_spectrum_material == NULL)
      log::ferror("A flattening of the FSR fluxes for a chi spectrum was "
...
#pragma omp for schedule(static)
    ...
    if (mpi::isSpatialDecomposed())
    {
      ...
      MPI_Allreduce(&_num_FSRs, &total_num_FSRs, 1, mpi::getDatatype<long>(), MPI_SUM, comm);
    }
#endif
    ...
    log::fdebug("Tot. Fiss. Src. = %f, Norm. factor = %f",
...
#pragma omp atomic update
    ...
    if (mpi::isSpatialDecomposed())
    {
      MPI_Allreduce(&num_negative_sources, &total_num_negative_sources, 1,
                    mpi::getDatatype<long>(), MPI_SUM, _geometry->getMPICart());
    ...
    if (total_num_negative_sources > 0 && !_negative_fluxes_allowed)
    {
      ...
      {
        log::fwarn("Computed %ld negative sources in %ld fsrs on %d "
                   "domains",
                   total_num_negative_sources,
                   total_num_negative_fsrs, total_num_negative_source_domains);
        if (iteration < 30)
          log::fwarn("Negative sources corrected to zero");
      }

      /* Output negative sources for debugging */
      if ((_print_negative_sources || log::get_level() == +log::level::debug) && _cmfd != NULL)
...
#pragma omp parallel for schedule(static)
      for (long r = 0; r < _num_FSRs; r++)
      {
        for (int e = 0; e < _NUM_GROUPS; e++)
          ...
          {
            residuals[r] += std::pow((_scalar_flux(r, e) - reference_flux(r, e)) /
                                         reference_flux(r, e),
                                     2);
          }
      }
      ...
      for (long r = 0; r < _num_FSRs; r++)
      {
        ...
        if (material->isFissionable())
        {
          ...
          if (old_fission_source > 0.)
            residuals[r] = std::pow((new_fission_source - old_fission_source) /
                                        old_fission_source,
                                    2);
        }
      }
...
#pragma omp parallel for private(new_total_source, old_total_source, \
      ...
      for (long r = 0; r < _num_FSRs; r++)
      {
        ...
        if (old_total_source > 0.)
          residuals[r] = std::pow((new_total_source - old_total_source) /
                                      old_total_source,
                                  2);
      }
    }
    ...
    if (mpi::isSpatialDecomposed())
    {
      ...
      long reduced_norm;
      MPI_Allreduce(&norm, &reduced_norm, 1, mpi::getDatatype<long>(), MPI_SUM, comm);
      norm = reduced_norm;
    }
#endif

    if (res_type == FISSION_SOURCE && norm == 0)
      log::ferror("The Solver is unable to compute a "
                  "FISSION_SOURCE residual without fissionable FSRs");

    /* Error check residual componenets */
    if (residual < 0.0)
    {
      log::fwarn("MOC residual mean square error %6.4f less than zero",
                 residual);
      residual = 0.0;
    }
    if (norm <= 0)
    {
      log::fwarn("MOC residual norm %d less than one", norm);
      norm = 1;
    }
...
#pragma omp parallel for schedule(static)
    ...
    {
      MPI_Allreduce(MPI_IN_PLACE, &rates[2], 1, MPI_DOUBLE, MPI_SUM,
                    mpi::getMPIComm());
    }
  ...
  void CPUSolver::transportSweep()
  {
    ...
    log::fdebug("Transport sweep with %d OpenMP threads", _num_threads);

    if (_cmfd != NULL && _cmfd->isFluxUpdateOn())
    {
      if (_cmfd->GetHexLatticeEnable())
        _cmfd->zeroHexCurrents();
      else
        _cmfd->zeroCurrents(); // 将CMFD单元的中子流以及涉及域通信的中子流归零
    }
    ...
    copyBoundaryFluxes();
#endif
    _timer->stopTimer("Copy Boundary Flux");

    /* Tally the starting fluxes to boundaries 暂时不看*/
    if (_cmfd != NULL)
    ...
    /* Tracks are traversed and the MOC equations from this CPUSolver are applied
       to all Tracks and corresponding segments 遍历轨迹，并将来自该CPUSolver的MOC方程应用于所有轨道和相应的分段*/
    if (_OTF_transport)
    {
      TransportSweepOTF sweep_tracks(_track_generator);
      sweep_tracks.setCPUSolver(this);
      sweep_tracks.execute(); // OTF输运求解过程
    }
    else
    {
...
#ifdef ENABLE_MPI_
    /* Synchronize flux across processes */
    if (mpi::isPrdTrackDecomposed())
    {
      _timer->startTimer();
      transferScalarFluxes();
      _timer->stopTimer("Transfer Scalar Flux");
    }
...
#pragma omp simd aligned(tau, delta_psi)
      for (int pe = 0; pe < num_polar_2 * _NUM_GROUPS; pe++)
      {
        ...
        delta_psi[pe] = (tau[pe] * track_flux[pe] - length *
                                                        _reduced_sources(fsr_id, pe % _NUM_GROUPS)) *
                        exponential;
...
#pragma omp simd aligned(fsr_flux)
      for (int pe = 0; pe < num_polar_2 * _NUM_GROUPS; pe++)
      {
        auto p = pe / _NUM_GROUPS;
        auto e = pe % _NUM_GROUPS;
...
#pragma omp flush
                                       ...
                                       float *track_flux)
  {
    ...
    /* 这里的角通量是延迟更新的。
     * 轨迹的角通量会赋值给_start_flux中的某个元素，而_start_flux这个数组
     * 只在一次输运扫描结束后（开始时）才会拷贝给_boundary_flux。
     */
...
#pragma omp atomic update
    ...
    if (total_num_negative_fluxes > 0 && !_negative_fluxes_allowed)
    {
      if (_geometry->isRootDomain())
      {
        log::fwarn("Computed %ld negative fluxes on %d domains",
  ...
  void CPUSolver::computeStabilizingFlux()
  {
    ...
    if (_stabilization_type == +stabilizationType::DIAGONAL)
    {
...
#pragma omp parallel for schedule(static)
      for (long r = 0; r < _num_FSRs; r++)
      {
        ...
        for (int e = 0; e < _NUM_GROUPS; e++)
        {
          ...
          if (sigma_s < 0.0)
            _stabilizing_flux(r, e) = -_scalar_flux(r, e) * _stabilization_factor * sigma_s / sigma_t[e];
        }
      }
    }
    else if (_stabilization_type == +stabilizationType::YAMAMOTO)
    {
...
#pragma omp parallel for schedule(static)
    ...
    else if (_stabilization_type == +stabilizationType::GLOBAL)
    {
...
#pragma omp parallel for schedule(static)
                                         ...
                                         bool nu)
  {
    ...
    if (_scalar_flux == NULL)
      log::ferror("Unable to compute FSR fission rates since the "
                  "source distribution has not been calculated");

    log::fverbose("Computing FSR fission rates...");
...
#pragma omp parallel for private(sigma_f, vol) schedule(static)
  ...
  void CPUSolver::printInputParamsSummary()
  {
    ...
    /* Print threads used */
    log::finfo("Using %d threads", _num_threads);
  }

#ifdef ENABLE_MPI_
  /// \brief Transfers scalar flux across processes.
  /// \details The buffer is _scalar_flux and it is synchronized in place.
  ///          So, _scalar_flux is assumed to be the contribution to scalar
  ///          fluxes during a single transport sweep. It this is not true,
  ///          it should be pre-processed to ensure the result of this
  ///          reduction. For example, dividing each of the scalar fluxes
  ///          by the number of processes before each transport sweep could
  ///          make this method work.
  void CPUSolver::transferScalarFluxes()
  {

    long num_flux = _num_FSRs * _num_groups;

    log::verbose_once("Reducing scalar fluxes of {} FSRs...", num_flux);

    MPI_Allreduce(MPI_IN_PLACE, _scalar_flux, num_flux,
                  mpi::getDatatype<FP_PRECISION>(), MPI_SUM,
                  mpi::getMPIComm());
  }
  ...
  void CPUSolver::printLoadBalancingReport()
  {
    ...
    long min_segments = num_segments;
    long max_segments = num_segments;
    long total_segments = num_segments;

    if (mpi::isSpatialDecomposed())
    {
      MPI_Comm_size(_geometry->getMPICart(), &num_ranks);
      MPI_Reduce(&num_segments, &min_segments, 1, mpi::getDatatype<long>(), MPI_MIN, 0,
                 _geometry->getMPICart());
      MPI_Reduce(&num_segments, &max_segments, 1, mpi::getDatatype<long>(), MPI_MAX, 0,
                 _geometry->getMPICart());
      MPI_Reduce(&num_segments, &total_segments, 1, mpi::getDatatype<long>(), MPI_SUM, 0,
                 _geometry->getMPICart());
    }
    FP_PRECISION mean_segments = float(total_segments) / num_ranks;
    log::verbose_once("Min / max / mean number of segments in domains: "
                      "{:.1e} / {:.1e} / {:.1e}",
    ...
    long sum_border_tracks_200 = std::max(FP_PRECISION(1),
                                 ...
                                 100.;
    log::verbose_once("Percentage of tracks exchanged in X/Y/Z direction: "
                      "{:.2f} / {:.2f} / {:.2f} %",
                      tracks_x / sum_border_tracks_200,
                      tracks_y / sum_border_tracks_200,
                      tracks_z / sum_border_tracks_200);
  }
#endif // ENABLE_MPI_
...
#pragma omp parallel for
    for (size_t r = 0; r < fsr_ids.size(); r++)
    {
      if (fsr_ids.at(r) != -1)
...
#endif
    if (!mpi::isSpatialDecomposed())
      for (size_t i = 0; i < fsr_ids.size(); i++)
...
#pragma omp parallel for
      ...
      if (rank == 0)
      {
        for (size_t i = 0; i < dim1.size(); i++)
        {
          for (size_t j = 0; j < dim2.size(); j++)
          {
            ...
            log::finfo("(%d: %f, %d: %f) -> %f", i, dim1.at(i), j,
                       dim2.at(j), flux);
          }
        }
      }
                                       ...
                                       int num_z)
  {
    ...
    int *by_group = new int[_NUM_GROUPS]();
