...
void CPUSolver::initializeFluxArrays() {
  ...
  try {
    ...
    max_size_mb = static_cast<double>(num_flux_arrays) * max_size
                  * sizeof(FP_PRECISION) / (1 << 20);
...
#pragma omp atomic update
  ...
  if (total_num_negative_sources > 0 && !_negative_fluxes_allowed) {
    ...
    if ((_print_negative_sources || log::get_level() == +log::level::debug)
        && _cmfd != NULL)
 ...
 *用于记录当前段（segment）在适当的CMFD网格单元表面上的贡献
 * @param curr_segment a pointer to the Track segment of interest
