...
Vector::Vector(omp_lock_t* cell_locks, int num_x, int num_y, int num_z,
  ...
  //TODO Change initialization of Vector to clean Valgrind log  将Vector的初始化更改为清理Valgrind日志
  setNumX(num_x);
  setNumY(num_y);
  setNumZ(num_z);
  setNumGroups(num_groups);
  _num_rows = (_num_x*_num_y*_num_z-empty_cells)*_num_groups;

  /* Initialize array and set all to 0.0 */
  _array = new CMFD_PRECISION[_num_rows];
  setAll(0.0);  //将_array数组中的每个元素都赋值0

  /* Set OpenMP locks for each Vector cell */
  if (cell_locks == NULL)
    log::ferror("Unable to create a Vector without an array of cell "
               "locks");
...
void Vector::incrementValue(int cell, int group, CMFD_PRECISION val) {
  ...
  if (cell >= _num_x*_num_y*_num_z || cell < 0)
    log::ferror("Unable to increment Vector value for cell %d"
               " which is not between 0 and %d", cell, _num_x*_num_y*_num_z-1);
  else if (group >= _num_groups || group < 0)
    log::ferror("Unable to increment Vector value for group %d"
...
void Vector::incrementValues(int cell, int group_first, int group_last,
  ...
  if (cell >= _num_x*_num_y*_num_z || cell < 0)
    log::ferror("Unable to increment Vector values for cell %d"
               " which is not between 0 and %d", cell, _num_x*_num_y*_num_z-1);
  else if (group_first >= _num_groups || group_first < 0)
    log::ferror("Unable to increment Vector values for first group %d"
               " which is not between 0 and %d", group_first, _num_groups-1);
  else if (group_last >= _num_groups || group_last < 0)
    log::ferror("Unable to increment Vector values for last group %d"
               " which is not between 0 and %d", group_last, _num_groups-1);
  else if (group_first > group_last)
    log::ferror("Unable to increment Vector values with first group %d"
...
void Vector::setValue(int cell, int group, CMFD_PRECISION val) {
  ...
  if (cell >= _num_x*_num_y*_num_z || cell < 0)
    log::ferror("Unable to set Vector value for cell %d"
               " which is not between 0 and %d", cell, _num_x*_num_y*_num_z-1);
  else if (group >= _num_groups || group < 0)
    log::ferror("Unable to set Vector value for group %d"
               " which is not between 0 and %d", group, _num_groups-1);
...
void Vector::setValues(int cell, int group_first, int group_last,
  ...
  if (cell >= _num_x*_num_y*_num_z || cell < 0)
    log::ferror("Unable to set Vector values for cell %d"
               " which is not between 0 and %d", cell, _num_x*_num_y*_num_z-1);
  else if (group_first >= _num_groups || group_first < 0)
    log::ferror("Unable to set Vector values for first group %d"
               " which is not between 0 and %d", group_first, _num_groups-1);
  else if (group_last >= _num_groups || group_last < 0)
    log::ferror("Unable to set Vector values for last group %d"
               " which is not between 0 and %d", group_last, _num_groups-1);
  else if (group_first > group_last)
    log::ferror("Unable to set Vector values with first group %d"
               " greater than last group %d", group_first, group_last);
...
void Vector::copyTo(Vector* vector) {
  std::copy(_array, _array + _num_rows, vector->getArray());//copy(first, last, destination);
}
...
void Vector::setNumX(int num_x) {
  ...
  if (num_x < 1)
    log::ferror("Unable to set Vector num x to non-positive value %d",
               num_x);
...
void Vector::setNumY(int num_y) {
  ...
  if (num_y < 1)
    log::ferror("Unable to set Vector num y to non-positive value %d",
               num_y);
...
void Vector::setNumZ(int num_z) {
  ...
  if (num_z < 1)
    log::ferror("Unable to set Vector num z to non-positive value %d",
               num_z);
...
void Vector::setNumGroups(int num_groups) {
  ...
  if (num_groups < 1)
    log::ferror("Unable to set Vector num groups to non-positive value"
               " %d", num_groups);
