...
namespace antmoc
{
   ...
   */
  Solver::Solver(TrackGenerator *track_generator) : _xs_log_level(+log::level::error),
                                                    _segment_formation(+segmentationType::EXPLICIT_2D),
                                                    _stabilization_type(+stabilizationType::DIAGONAL)
  {
  ...
  /**
   * @brief The version for smart pointers
   */
  Solver::Solver(TrackGeneratorPtr track_generator)
      : Solver(track_generator.get())
  {
    // Hold the shared pointer
    _track_generator_smart = track_generator;
  }
  ...
  Solver::~Solver()
  {
    ...
    for (size_t i = 0; i < _groupwise_scratch.size(); i++)
  ...
  Geometry *Solver::getGeometry()
  {
    ...
    if (_geometry == NULL)
      log::ferror("Unable to return the Solver's Geometry since it "
  ...
  TrackGenerator *Solver::getTrackGenerator()
  {
    ...
    if (_track_generator == NULL)
      log::ferror("Unable to return the Solver's TrackGenetrator "
  ...
  FP_PRECISION Solver::getFSRVolume(long fsr_id)
  {
    ...
    if (fsr_id < 0 || fsr_id > _num_FSRs)
      log::ferror("Unable to get the volume for FSR %d since the FSR "
                  "IDs lie in the range (0, %d)",
                  fsr_id, _num_FSRs);

    else if (_FSR_volumes == NULL)
      log::ferror("Unable to get the volume for FSR %d since the FSR "
                  "volumes have not yet been computed",
                  fsr_id);
  ...
  int Solver::getNumEnergyGroups()
  {
    return _num_groups;
  }

  float *Solver::getBoundaryFlux(long track_id, bool fwd)
  {
  ...
  double Solver::getFlux(long fsr_id, int group)
  {
    ...
    if (fsr_id >= _num_FSRs)
      log::ferror("Unable to return a scalar flux for FSR ID = %d "
                  "since the max FSR ID = %d",
                  fsr_id, _num_FSRs - 1);

    else if (fsr_id < 0)
      log::ferror("Unable to return a scalar flux for FSR ID = %d "
                  "since FSRs do not have negative IDs",
                  fsr_id);

    else if (group - 1 >= _num_groups)
      log::ferror("Unable to return a scalar flux in group %d "
                  "since there are only %d groups",
                  group, _num_groups);

    else if (group <= 0)
      log::ferror("Unable to return a scalar flux in group %d "
                  "since groups must be greater or equal to 1",
                  group);

    else if (_scalar_flux == NULL)
      log::ferror("Unable to return a scalar flux "
  ...
  double Solver::getFSRSource(long fsr_id, int group)
  {
    ...
    if (fsr_id >= _num_FSRs)
      log::ferror("Unable to return a source for FSR ID = %d "
                  "since the max FSR ID = %d",
                  fsr_id, _num_FSRs - 1);
      ...
      log::ferror("Unable to return a source in group %d "
                  "since there are only %d groups",
                  group, _num_groups);
      ...
      log::ferror("Unable to return a source "
                  "since it has not yet been computed");
  ...
  void Solver::setGeometry(Geometry *geometry)
  {
    ...
    if (geometry->getNumFSRs() == 0)
      log::ferror("Unable to set the Geometry for the Solver since the "
  ...
  void Solver::setTrackGenerator(TrackGenerator *track_generator)
  {
    ...
    if (!track_generator->containsSegments())
      log::ferror("Unable to set the TrackGenerator for the Solver "
    ...
    {
      // 3D情况下，每个极角都有轨迹，因此每条轨迹角通量只按能群变化
      _fluxes_per_track = _num_groups;
      _tot_num_tracks = track_generator_3D->getMyNum3DTracks();
    ...
    {
      // 2D情况下，极角方向没有实际的轨迹，因此每条2D轨迹都承担所有极角的角通量
      _fluxes_per_track = _num_groups * _num_polar / 2;
      _tot_num_tracks = _track_generator->getMyNum2DTracks();
#ifdef THREED
      log::error("The code has been compiled for 3D cases only, please "
  ...
  void Solver::setConvergenceThreshold(double threshold)
  {
    ...
    if (threshold <= 0.0)
      log::ferror("Unable to set the convergence threshold to %f "
                  "since it is not a positive number",
                  threshold);
  ...
  void Solver::setFixedSourceByFSR(long fsr_id, int group, double source)
  {
    ...
    if (group <= 0 || group > _num_groups)
      log::ferror("Unable to set fixed source for group %d in "
                  "in a %d energy group problem",
                  group, _num_groups);

    if (fsr_id < 0 || fsr_id >= _num_FSRs)
      log::ferror("Unable to set fixed source for FSR %d with only "
  ...
  void Solver::setSolverMode(solverMode solver_mode)
  {
    ...
    if (solver_mode == ADJOINT)
      log::finfo("Solver set to perform an adjoint calculation");
  }
  ...
  void Solver::setRestartStatus(bool is_restart)
  {
    ...
    if (is_restart)
      log::finfo("Solver is in restart mode, no fluxes will be reset.");
  ...
  /**
   * @brief Directs the solver to correct unphysical cross-sections.
   ...
   */
  void Solver::setCorrectXS(bool correct_xs)
  {
    _correct_xs = correct_xs;
  }

  /// \brief Checks MGXS.
  void Solver::setCheckXS(bool check_xs)
  {
    _check_xs = check_xs;
  }
   ...
   */
  void Solver::setCheckXSLogLevel(log::level log_level)
  {
    _xs_log_level = log_level;
  }

  /**
   * @brief Directs the solver to use the diagonal stabilizing correction to
  ...
  /**
   * @brief Instructs the solver to perform an initial spectrum calculation
  ...
  void Solver::initializeExpEvaluators()
  {
    ...
    log::info("Initializing exponential interpolation table...");
    ...
    if (max_tau_a > max_tau_b)
    {
      ...
      log::node("Splitting segments since the maximum optical length in the domain "
                "is {} and the maximum supported by the exponential evaluator is {}.",
                max_tau_a, max_tau_b);
    ...
    // Report temporary segments storage because segments may have been counted again.
    _track_generator->reportTemporarySegmentsStorage();
    ...
    _timer->stopTimer("Initialize ExpEvaluators");
  }
  ...
  void Solver::initializeFSRs()
  {
    ...
    log::info("Initializing solver arrays...");
  ...
  void Solver::countFissionableFSRs()
  {
    ...
    log::verbose_once("Counting fissionable FSRs...");
  ...
  void Solver::checkLimitXS(int iteration)
  {
    ...
    if (iteration == _reset_iteration)
      log::finfo("Re-setting material cross-sections");
    else
      return;
    ...
    log::finfo("Material re-set complete");
  }
  ...
  void Solver::limitXS()
  {
    ...
    log::finfo("Limiting negative cross-sections in %d materials",
               _limit_xs_materials.size());
    auto &all_materials = _geometry->getAllMaterials();
    for (size_t i = 0; i < _limit_xs_materials.size(); i++)
    {
    ...
    log::finfo("Cross-section adjustment complete");
  }
  ...
  void Solver::checkXS()
  {
    ...
    log::info("Checking material cross-sections...");
...
#pragma omp critical
    ...
    /* Check all unique materials */
    for (auto &material : materials_set)
    {
      ...
      for (int e = 0; e < _num_groups; e++)
      {
        ...
        for (int g = 0; g < _num_groups; g++)
        {
          ...
          if (scattering_matrix[g * _num_groups + e] < 0)
            log_print(_xs_log_level,
                      "In material '{}' ({}), group {}->{}: negative scattering cross-section "
                      "encountered",
                      material->getName(), material->getId(), e, g);
        }
        if (sigma_s > sigma_t[e])
        {
          auto msg = fmt::format(
              "In material '{}' ({}), group {}: invalid cross-sections encountered. "
              "The scattering cross-section has value {:6.4f} which is greater than "
              "the total cross-section of value {:6.4f}",
              material->getName(), material->getId(), e, sigma_s, sigma_t[e]);
          ...
          if (_correct_xs)
          {
            ...
            log::warn(msg.c_str());
            sigma_t[e] = sigma_s;
            log::warn("In material '{}' ({}), group {}: the total cross-section has been "
                      "corrected to {:6.4f}",
                      material->getName(), material->getId(), e,
                      sigma_s);
          }
          else
          {
            log_print(_xs_log_level, msg.c_str());
          }
        }
        ...
        if (sigma_t[e] < 0 || sigma_f[e] < 0 || nu_sigma_f[e] < 0 || chi[e] < 0)
          log_print(_xs_log_level,
                    "In material '{}' ({}), group {}: negative cross-section encountered",
                    material->getName(), material->getId(), e);
      }
    }
    log::info("Completed material cross-section checks");
  }
  ...
  void Solver::initializeFixedSources()
  {
    ...
    log::fverbose("Transferring fixed sources from cells/materials to FSRs");
  ...
  /**
   * @brief Initializes a Cmfd object for acceleration prior to source iteration.在源迭代之前初始化Cmfd对象以进行加速
  ...
  void Solver::initializeCmfd()
  {
    ...
    {                                 // 0000000000000000000
      ...
      int group_count = 0;
      for (int i = 0; i < hex_num_groups; i++)
      {
        if (i < hex_num_groups - 1)
        {
          for (int g = group_count; g < group_count + _num_groups / hex_num_groups; g++)
          {
            cmfd_group_structure.at(i).push_back(g + 1);
          }
        }
        else
        {
          for (int g = group_count; g < _num_groups; g++)
          {
            cmfd_group_structure.at(i).push_back(g + 1);
          }
        }
        group_count = group_count + _num_groups / hex_num_groups;
      }
  ...
  void Solver::calculateInitialSpectrum(double threshold)
  {
    ...
    log::finfo("Calculating initial spectrum with threshold %3.2e",
               threshold);
    ...
    /* Solve the system */
    log::finfo("Computing K-eff");
    _k_eff = spectrum_calculator.computeKeff(0);
    log::finfo("Normalizing Fluxes");
    normalizeFluxes();
    ...
    log::finfo("Calculated initial spectrum with k-eff = %6.6f", _k_eff);
  }
  ...
  void Solver::resetMaterials(solverMode mode)
  {
    ...
    log::fverbose("Resetting materials...");
  ...
  void Solver::computeFlux(int max_iters, bool only_fixed_source)
  {
    ...
    if (_track_generator == NULL)
      log::ferror("The Solver is unable to compute the flux "
                  "since it does not contain a TrackGenerator");

    log::finfo("Computing the flux...");
    ...
    for (int i = 0; i < max_iters; i++)
    {
      ...
      _num_iterations++;
      log::finfo("Iteration %d:\tres = %1.3E", i, residual);
    ...
    if (_num_iterations == max_iters && residual >= _converge_thresh)
    {
      log::fwarn("Unable to converge the flux");
    }

    /* Total time to converge the source */
    _timer->stopTimer("Source Iteration");

    /* Total time of computing flux */
    _timer->stopTimer("Total time");
  }
   ...
   *          are all supported options in the code at this time.
   *
  ...
  void Solver::computeSource(int max_iters, double k_eff, residualType res_type)
  {
    ...
    if (_track_generator == NULL)
      log::ferror("The Solver is unable to compute the source "
                  "since it does not contain a TrackGenerator");

    else if (k_eff <= 0.)
      log::ferror("The Solver is unable to compute the source with "
                  "keff = %f since it is not a positive value",
                  k_eff);

    log::finfo("Computing the source...");
    ...
    if (_num_iterations == max_iters && residual >= _converge_thresh)
    {
      log::warn("Unable to converge the source");
    }

    /* Total time to converge the source */
    _timer->stopTimer("Source Iteration");

    /* Total time of computing source */
    _timer->stopTimer("Total time");
  }

  /**
   * @brief Computes keff by performing a series of transport sweep and
   *        source updates.通过执行一系列传输扫描和源更新来计算keff
   * @details This is the main method exposed to the user through the Python
  ...
  void Solver::computeEigenvalue(int max_iters, residualType res_type)
  {
    ...
    if (_track_generator == NULL)
      log::ferror("The Solver is unable to compute the eigenvalue "
                  "since it does not contain a TrackGenerator");

    log::finfo("Initializing MOC eigenvalue solver...");

    /* Clear all timing data from a previous simulation run 清除上一次模拟运行中的所有计时数据*/
    clearTimerSplits();

    /* Start the timer */
    _timer->startTimer();

    /* Reset number of iterations, start at 1 if restarting 重置迭代次数，如果重新启动，则从1开始*/
    ...
    {
      _cmfd->setConvergenceData(&convergence_data); // 设置CMFD的收敛数据信息
      log::info("iter   k-eff   eps-k  eps-MOC   D.R.   "
                "eps-FS1   eps-FSN   #FS  eps-flux1 eps-fluxN"
                "  #FX1 #FXN  MAX P.F.");
    }

    /* Record the initialization time */
    _timer->stopTimer("Initialize Solver");
    ...
    for (int i = 0; i < max_iters; i++)
    {
      ...
      if (_verbose && _cmfd != NULL)
      {
        ...
        log::finfo("%3d  %1.6f  %5d  %1.6f  %1.3f  %1.6f  %1.6f"
                   "  %3d  %1.6f  %1.6f  %3d  %3d    %.4e",
                   i, _k_eff,
      ...
      {
        log::finfo("Iteration %d:  k_eff = %1.6f   "
                   "res = %1.3E  delta-k (pcm) = %d D.R. = %1.4f",
                   i, _k_eff,
                   residual, dk, dr);
      }
    ...
    if (_num_iterations == max_iters)
      log::warn_once("Unable to converge the source distribution");

    /* Total time to converge the source */
    _timer->stopTimer("Source Iteration");

    /* Total time of computing eigenvalue */
    _timer->stopTimer("Total time");
  }

  /**
   * @brief Load or compute a starting guess for scalar fluxes.
   * @details By default, it is assumed that an initial flux guess flat in space
   *          and energy. Other options are available:
  ...
  void Solver::computeInitialFluxGuess(bool is_source_computation)
  {
    ...
    if (_load_initial_FSR_fluxes)
    {
      ...
      {
        log::info("Startup sweep {} / {}", i, startup_iterations);
        transportSweep();
      }
      addSourceToScalarFlux();
    }
   ...
   */
  void Solver::setOTFTransport()
  {
    _OTF_transport = true;
    log::finfo("Using On-The-Fly transport");
  }
  ...
  void Solver::printTimerReport()
  {
    ...
    const std::string source_string = "Source Iteration";
    const std::string sweep_string = "Transport Sweep";
    ...
    log::header("Timing Report - Solution (average)");

    /* Get the total runtime of solution */
    _timer->printSplit("Total time", "Total time to solution");
    _timer->printSplit("Initialize Solver", "Solver Initialization", 1, "Total time");
    _timer->printSplit("Initialize FSRs", "FSR Volumes & Centroids", 2, "Initialize Solver");

#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    {
      _timer->printSplit("Transfer FSRs", "FSRs Data Transfer", 3, "Initialize FSRs");
    }
#endif

    _timer->printSplit("Initialize ExpEvaluators", "ExpEvaluator Initialization",
                       2, "Initialize Solver");

    /* Get the total runtime of source iteration */
    _timer->printSplit(source_string, 1, "Total time");

    /* Time per iteration */
    msg_string = "Time per iteration";
    double source_time = _timer->getSplit(source_string);
    _timer->appendSplit(msg_string, source_time / _num_iterations);
    _timer->printSplit(msg_string, 2);

    /* Time to update values before or after transport sweep */
    _timer->printSplit("Update Solver Values", "Source & FSRs & keff Update", 2, source_string);
    _timer->printSplit("Transport Sweep", "Transport Sweep", 2, source_string);
    _timer->printSplit("Copy Boundary Flux", "Boundary Flux Update", 3, sweep_string);

#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    {
      _timer->printSplit("Transfer Scalar Flux", "Scalar Flux transfer", 3, sweep_string);
    }

    if (mpi::isSpatialDecomposed())
    {
      const std::string transfer_string = "Total transfer time";
      _timer->printSplit("Idle time", "Total Idle Time Between Sweeps", 3, sweep_string);
      _timer->printSplit("Total transfer time", "Angular Flux Transfer", 3, sweep_string);
      _timer->printSplit("Packing time", "Packing Time", 4, transfer_string);
      _timer->printSplit("Communication time", "Communication Time", 4, transfer_string);
      _timer->printSplit("Unpacking time", "Unpacking Time", 4, transfer_string);
    }
#endif
    ...
    long total_num_tracks = _track_generator->getNumTracks();
#ifdef ENABLE_MPI_
    if (mpi::isPrdTrackDecomposed())
    {
      MPI_Allreduce(&num_segments, &total_num_segments, 1, mpi::getDatatype<long>(),
    ...
    if (mpi::isSpatialDecomposed())
    {
      ...
      MPI_Allreduce(&num_segments, &total_num_segments, 1, mpi::getDatatype<long>(),
                    MPI_SUM, MPI_cart);
      MPI_Allreduce(&_tot_num_tracks, &total_num_tracks, 1, mpi::getDatatype<long>(),
                    MPI_SUM, MPI_cart);
    }
#endif
    ...
    long num_integrations = 2 * _fluxes_per_track * total_num_segments * _num_iterations;
    double transport_sweep = _timer->getSplit(sweep_string);
    _timer->appendSplit(msg_string, transport_sweep / num_integrations);
    _timer->printSplit(msg_string);

    if (_cmfd != NULL)
      _cmfd->printTimerReport();

    /* Show the statistics of objects */
    printObjectStatistics(total_num_tracks, total_num_segments, _geometry->getNumTotalFSRs());
  }

  /**
   * @brief Prints statistics for important objects
   */
  void Solver::printObjectStatistics(long tracks, long segments, long fsrs)
  {
    ...
    log::result("{1: ^{0}}{2: ^{0}}{3: ^{0}}",
                part_length, "# tracks", "# segments", "# FSRs");
  ...
  void Solver::printFissionRates(std::string fname, int nx, int ny, int nz)
  {
    ...
    if (rank == 0)
    {
      ...
      out << "Fission rates for (" << nx << ", " << ny << ", " << nz << ")" << std::endl;
      for (int i = 0; i < nx; i++)
      {
        for (int j = 0; j < ny; j++)
        {
          for (int k = 0; k < nz; k++)
          {
            ...
            out << "Region " << i << ", " << j << ", " << k << " at point " << "(" << x_min + (i + 0.5) * (x_max - x_min) << ", " << y_min + (j + 0.5) * (y_max - y_min) << ", " << z_min + (k + 0.5) * (z_max - z_min) << ") -> " << fission_rates[ind] << std::endl;
          }
        }
      }
    }
  ...
  void Solver::dumpFSRFluxes(std::string fname)
  {
    ...
    if (out == NULL)
      log::error("Fluxes file %s could not be written.", &filename[0]);
                             ...
                             double tolerance)
  {
    ...
    if (in == NULL)
      log::ferror("Failed to find file %s", filename.c_str());
    log::finfo("Reading fluxes from %s", fname.c_str());
    ...
    if (assign_k_eff)
    {
      log::finfo("Loaded k-eff %6.6f", k_eff);
      _k_eff = k_eff;
    }
    ...
    if (num_FSRs != _num_FSRs)
      log::ferror("The number of FSRs in the current Geometry do not match"
                  " the number of FSRs in the binary flux data file");
    if (num_groups != _num_groups)
      log::ferror("The number of energy groups in the current Geometry do "
...
#pragma omp parallel for
    for (long r = 0; r < _num_FSRs; r++)
    {
      ...
      for (int dx = -1; dx <= 1; dx++)
      {
        for (int dy = -1; dy <= 1; dy++)
        {
          for (int dz = -1; dz <= 1; dz++)
          {
            ...
            if (hashed_lookup.find(index) != hashed_lookup.end())
            {
              for (size_t j = 0; j < hashed_lookup[index].size(); j++)
              {
      ...
      if (load_fsr < 0)
        log::ferror("Loaded FSR %d with location (%3.2f, %3.2f, %3.2f) "
                    "and cell (%d, %d, %d)",
                    load_fsr, centroid_xyz[0],
    ...
    log::fverbose("Status of loading FSR fluxes: %d", ret);
    log::finfo("FSR fluxes successfully loaded with maximum centroid "
               "error %6.4e",
               max_centroid_error);
  }
  ...
  void Solver::printInputParamsSummary()
  {
    ...
    log::info("Number of azimuthal angles = {}", _quad->getNumAzimAngles());
    log::info("Azimuthal ray spacing = {}", _track_generator->getDesiredAzimSpacing());
    log::info("Number of polar angles = {}", _quad->getNumPolarAngles());

    if (_SOLVE_3D)
    {
      TrackGenerator3D *track_generator_3D =
          static_cast<TrackGenerator3D *>(_track_generator);
      log::info("Z-spacing = {}", track_generator_3D->getDesiredZSpacing());
    }

    /* Print source type */
    log::info("Source type = {}", _source_type.c_str());

    /* Print MOC stabilization */
    if (_stabilize_transport)
    {

      log::info("MOC Damping = {} ({:3.2f})", _stabilization_type._to_string(),
                _stabilization_factor);
    }
    else
    {
      log::info("MOC transport undamped");
    }
    ...
    if (_cmfd != NULL)
      ...
      log::info("CMFD acceleration: OFF");
  }
 ...
 */
  ...
  void Solver::printBGQMemory()
  {
    ...
    log::finfo("Allocated heap: %.2f MB, avail. heap: %.2f MB",
               (double)heap / (1024 * 1024), (double)heapavail / (1024 * 1024));
    log::finfo("Allocated stack: %.2f MB, avail. stack: %.2f MB",
               (double)stack / (1024 * 1024), (double)stackavail / (1024 * 1024));
    log::finfo("Memory: shared: %.2f MB, persist: %.2f MB, guard: %.2f "
               "MB, mmap: %.2f MB\n",
