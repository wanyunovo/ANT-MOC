...
namespace antmoc
{
   ...
   */
  CPULSSolver::CPULSSolver(TrackGeneratorPtr track_generator)
      : CPULSSolver(track_generator.get())
  {
  ...
  void CPULSSolver::initializeFluxArrays()
  {
    ...
    try
    {
      ...
      log::finfo("Max linear flux storage per domain = %6.2f MiB",
                 max_size_mb);
    ...
    {
      log::ferror("Could not allocate memory for the scalar flux moments");
    }
  }
  ...
  void CPULSSolver::initializeSourceArrays()
  {
    ...
    try
    {
      ...
      if (_fixed_source_moments_on)
        max_size_mb *= 3;
      log::finfo("Max linear source storage per domain = %6.2f MiB",
                 max_size_mb);
    ...
    {
      log::ferror("Could not allocate memory for FSR source moments");
    }
  }
  ...
  void CPULSSolver::initializeFSRs()
  {
    ...
    log::finfo("Generating linear expansion coefficients");
    LinearExpansionGenerator lin_src_coeffs(this);
    lin_src_coeffs.execute();
    log::finfo("Linear expansion coefficient generation complete");
  }
  ...
  void CPULSSolver::initializeFixedSources()
  {
    ...
    log::finfo("Initializing linear fixed sources...");
         ...
         fsr_iter != _fix_src_xyz_FSR_map.end(); ++fsr_iter)
    {
      ...
      if ((fabs(_fixed_sources_xyz(fsr_id, group, 0)) > FLT_EPSILON &&
        ...
        log::warn("Overriding fixed linear source %f %f %f in FSR ID=%d"
                  " group %d with %f %f %f",
                  _fixed_sources_xyz(fsr_id, group, 0),
...
#pragma omp parallel // 2. 开启 OpenMP 并行计算
    {
...
#pragma omp for schedule(guided)
      for (long r = 0; r < _num_FSRs; r++)
      {
        ...
        material = _FSR_materials[r];    // 获取当前区域的材料
        sigma_s = material->getSigmaS(); // 获取散射截面数组

        for (int g = 0; g < _NUM_GROUPS; g++) // 遍历每一个能群 g
        {
...
#pragma omp simd aligned(tau, src_flat, src_linear, fsr_flux, exp_G, fsr_flux_x, fsr_flux_y, fsr_flux_z)
        for (int e = start_vector; e < start_vector + VEC_LENGTH; e++)
        {
          ...
          FP_PRECISION delta_psi = (tau[e] * track_flux[e] - length * src_flat[e]) * exp_F1 - src_linear[e] * length * length * exp_F2;
...
#pragma omp simd aligned(tau, src_flat, src_linear, delta_psi, exp_F1, exp_F2, exp_H)
      for (int pe = 0; pe < num_polar_2 * _NUM_GROUPS; pe++)
      {
        ...
        // Compute the change in flux across the segment
        delta_psi[pe] = (tau[pe] * track_flux[pe] - length * src_flat[pe % _NUM_GROUPS]) * exp_F1[pe] - length * length * src_linear[pe] * exp_F2[pe];
...
#pragma omp for
      for (long r = 0; r < _num_FSRs; r++)
      {
        ...
        for (int e = 0; e < _NUM_GROUPS; e++)
        {
          ...
          _scalar_flux_xyz(r, e, 0) += flux_const * _reduced_sources_xyz(r, e, 0) * _FSR_source_constants[r * _NUM_GROUPS * nc + e];
          _scalar_flux_xyz(r, e, 0) += flux_const * _reduced_sources_xyz(r, e, 1) * _FSR_source_constants[r * _NUM_GROUPS * nc + 2 * _NUM_GROUPS + e];

          _scalar_flux_xyz(r, e, 1) /= volume;
          _scalar_flux_xyz(r, e, 1) += flux_const * _reduced_sources_xyz(r, e, 0) * _FSR_source_constants[r * _NUM_GROUPS * nc + 2 * _NUM_GROUPS + e];
          _scalar_flux_xyz(r, e, 1) += flux_const * _reduced_sources_xyz(r, e, 1) * _FSR_source_constants[r * _NUM_GROUPS * nc + _NUM_GROUPS + e];

          if (_SOLVE_3D)
          {
            _scalar_flux_xyz(r, e, 0) += flux_const * _reduced_sources_xyz(r, e, 2) * _FSR_source_constants[r * _NUM_GROUPS * nc + 3 * _NUM_GROUPS + e];
            _scalar_flux_xyz(r, e, 1) += flux_const * _reduced_sources_xyz(r, e, 2) * _FSR_source_constants[r * _NUM_GROUPS * nc + 4 * _NUM_GROUPS + e];

            _scalar_flux_xyz(r, e, 2) /= volume;
            _scalar_flux_xyz(r, e, 2) += flux_const * _reduced_sources_xyz(r, e, 0) * _FSR_source_constants[r * _NUM_GROUPS * nc + 3 * _NUM_GROUPS + e];
            _scalar_flux_xyz(r, e, 2) += flux_const * _reduced_sources_xyz(r, e, 1) * _FSR_source_constants[r * _NUM_GROUPS * nc + 4 * _NUM_GROUPS + e];
            _scalar_flux_xyz(r, e, 2) += flux_const * _reduced_sources_xyz(r, e, 2) * _FSR_source_constants[r * _NUM_GROUPS * nc + 5 * _NUM_GROUPS + e];
          }
...
#pragma omp parallel for
      for (long r = 0; r < _num_FSRs; r++)
        for (int e = 0; e < _NUM_GROUPS; e++)
          for (int i = 0; i < 3; i++)
            _stabilizing_flux_xyz(r, e, i) = _scalar_flux_xyz(r, e, i) * mult_factor;
    }
  }
...
#pragma omp critical
...
#endif
      double max_size_mb = (double)(max_size * sizeof(FP_PRECISION)) / (double)(1 << 20);
      log::finfo("Max linear constant storage per domain = %6.2f MiB",
                 max_size_mb);
   ...
   */
  void CPULSSolver::transferScalarFluxes()
  {

    CPUSolver::transferScalarFluxes();

    long num_flux = _num_FSRs * _NUM_GROUPS * 3;

    log::verbose_once("Reducing {} XYZ fluxes...", num_flux);

    MPI_Allreduce(MPI_IN_PLACE, this->_scalar_flux_xyz, num_flux,
