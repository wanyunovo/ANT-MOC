...
#pragma omp parallel
  ...
  if (mpi::isDomainDecomposed()) {
    mpi::mpiAllreduce(&_min_tau, &global_min_tau, 1, MPI_FLOAT, MPI_MIN);
    mpi::mpiAllreduce(&_max_tau, &global_max_tau, 1, MPI_FLOAT, MPI_MAX);
  }
#endif
  log::verbose_once("Min/max optical lengths in geometry {:.2e} / {:.2e}",
...
long SegmentCounter::getTotalNumSegments() {
  if (!_total_segments_counted)
    log::error("The total number of segments have not been counted. "
...
void CentroidGenerator::onTrack(Track* track, segment* segments) {
  ...
  if (track_3D != NULL) {
    ...
    if (_segment_formation == +segmentationType::OTF_STACKS) {
      long xy_index = track->getXYIndex();
...
#pragma omp parallel for
    for (long r=0; r < num_FSRs; r++) {
      ...
      if (std::abs(det) < MIN_DET || volume < 1e-6) {
        if (volume > 0)
          log::fdebug("Unable to form linear source components in "
                     "source region %d : determinant %.2e volume %.2e", r, det, volume);
...
#pragma omp parallel for
  ...
  if (max_ilem > 1e10)
    log::fverbose("Max inverse linear expansion matrix coeff %e", max_ilem);
  ...
  if (mpi::isSpatialDecomposed()) {
    MPI_Allreduce(&_num_flat, &total_num_flat, 1, MPI_INT, MPI_SUM,
                  geometry->getMPICart());
    MPI_Allreduce(&num_FSRs, &total_num_FSRs, 1, mpi::getDatatype<long>(), MPI_SUM,
                  geometry->getMPICart());
  }
#endif
  if (total_num_flat > 0)
    if (geometry->isRootDomain())
      log::fwarn("Unable to form linear source components in %d / %d "
...
#ifndef THREED
      if (track_3D == NULL) {
        for (size_t p=0; p < _quadrature->getNumPolarAngles()/2; p++) {
...
void TransportSweep::onTrack(Track* track, segment* segments) {
...
#ifndef NGROUPS
  _NUM_GROUPS = _cpu_solver->getNumEnergyGroups();
#endif
#ifndef LINEARSOURCE
  int num_moments = 1;
  if (_ls_solver != NULL)
  ...
  for (int s=0; s < num_segments; s++) {
    ...
    /* _track_idx 用于索引轨迹堆中的线段，因为转入的是轨迹堆的第一条轨迹。
     * 在非OTF-By-Stack方法中，这个索引值恒为0
     */
    segment* curr_segment = &segments[s];
...
#ifndef LINEARSOURCE
    if (_ls_solver == NULL)
      _cpu_solver->tallyScalarFlux(curr_segment, azim_index, fsr_flux, track_flux);
    else
#endif
      _ls_solver->tallyLSScalarFlux(curr_segment, azim_index,
  ...
  // 延迟更新，forward角通量并没有给_boundary_flux，而是临时放在_start_flux
  for (int i=0; i <= max_track_index; i++) {
...
#ifndef ONLYVACUUMBC
  /* Transfer boundary angular flux to outgoing Track */
  // 延迟更新，backward角通量并没有给_boundary_flux，而是临时放在_start_flux
  for (int i=0; i <= max_track_index; i++) {
...
void ReadSegments::onTrack(Track* track, segment* segments) {
  ...
  int num_segments;
  geometry->twiddleRead(&num_segments, sizeof(int), 1, _in);
  ...
  for (int s=0; s < num_segments; s++) {
    ...
    double length;
    geometry->twiddleRead(&length, sizeof(double), 1, _in);
    int material_id;
    geometry->twiddleRead(&material_id, sizeof(int), 1, _in);
    long region_id;
    geometry->twiddleRead(&region_id, sizeof(long), 1, _in);
    int  track_idx;
    geometry->twiddleRead(&track_idx, sizeof(int), 1, _in);
    double start_x;
    geometry->twiddleRead(&start_x, sizeof(double), 1, _in);
    double start_y;
    geometry->twiddleRead(&start_y, sizeof(double), 1, _in);
    double start_z;
    geometry->twiddleRead(&start_z, sizeof(double), 1, _in);
    ...
    if (cmfd != NULL) {
      ...
      geometry->twiddleRead(&cmfd_surface_fwd, sizeof(int), 1, _in);
      curr_segment._cmfd_surface_fwd = cmfd_surface_fwd;
      int cmfd_surface_bwd;
      geometry->twiddleRead(&cmfd_surface_bwd, sizeof(int), 1, _in);
 ...
 * 执行时，内核在所有轨道上循环，生成轨道并求解MOC方程。
 */
void TransportSweepOTF::execute() {
#pragma omp parallel
  {
    //使用 kernels 时，其实是像工厂方法一样使用它们共同的接口。代码中使用模板来取用 kernels,每个线程也会有自己的 kernel
    TransportKernel kernel(_track_generator);   
    kernel.setCPUSolver(_cpu_solver);
    loopOverTracksByStackTwoWay(&kernel);  //使用轨迹堆来遍历3D轨迹
  }
}
