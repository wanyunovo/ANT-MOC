 ...
 */
void ExpEvaluator::setQuadrature(QuadraturePtr quadrature) {
...
void ExpEvaluator::setMaxOpticalLength(FP_PRECISION max_optical_length) {
  ...
  if (max_optical_length <= 0)
    log::ferror("Cannot set max optical length to %f because it "
...
void ExpEvaluator::setExpPrecision(FP_PRECISION exp_precision) {
  ...
  if (exp_precision <= 0)
    log::ferror("Cannot set exp precision to %f because it "
...
FP_PRECISION ExpEvaluator::getTableSpacing() {
  ...
  if (_exp_table == NULL)
    log::ferror("Unable to return the exponential table spacing "
...
int ExpEvaluator::getTableSize() {
  ...
  if (_exp_table == NULL)
    log::ferror("Unable to return exponential table size "
               "since it has not yet been initialized");
...
FP_PRECISION* ExpEvaluator::getExpTable() {
  ...
  if (_exp_table == NULL)
    log::ferror("Unable to return exponential table "
               "since it has not yet been initialized");
...
void ExpEvaluator::initialize(int azim_index, int polar_index, bool solve_3D) {
  ...
  if (_quadrature == NULL)
    log::ferror("A Quadrature must be set before an Exponential "
  ...
  if (azim_index < 0 || azim_index > num_azim / 4)
    log::ferror("Invalid azimuthal angle index of %d. The index must be "
               "betwen 0 and %d", azim_index, num_azim/4);

  /* Check for a valid polar angle index */
  if (polar_index < 0 || polar_index > num_polar / 2)
    log::ferror("Invalid polar angle index of %d. The index must be "
  ...
  if (_interpolate) {
    log::fwarn_once("Interpolation tables are commented out in source"
                " code for optimization purposes");
    return;
  }

  log::fdebug("Initializing exponential interpolation table...");
  ...
  if (num_array_values > 1e5) {
    log::fwarn("Reducing exponential table size from %d to %d",
               num_array_values, 1e5);
    num_array_values = 1e5;
  }

  log::fverbose_once("Creating exponential lookup table with %d interpolation "
