...
VolumeKernel::VolumeKernel(TrackGenerator* track_generator) :
  ...
  if (_FSR_locks == NULL)
    log::ferror("Unable to create a VolumeKernel without first creating "
               "FSR locks");
...
/**
 * @brief Apply MOC equations, tally fluxes and tally CMFD currents  应用MOC方程，计算通量和计算CMFD电流.
...
void TransportKernel::execute(FP_PRECISION length, Material* mat, long fsr_id,
  ...
  for (int i=0; i < num_cuts; i++) {
    ...
    /* Apply MOC equations */
    _cpu_solver->tallyScalarFlux(&curr_segment, _azim_index, fsr_flux, track_flux);
