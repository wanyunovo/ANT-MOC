...
void TraverseSegments::traceStackOTF(Track* flattened_track, int polar_index,
  ...
  size_t num_fsrs = 0;
  double* axial_mesh;
  if (_global_z_mesh != nullptr) {  //如果有全局轴向网
    num_fsrs = _mesh_size;
    axial_mesh = _global_z_mesh;
  }
  ...
  segment* segments_2D = flattened_track->getSegments();
  for (int s=0; s < flattened_track->getNumSegments(); s++) {  //遍历2D线段，一次处理一个轴向挤出FSR内的线段
    ...
    for (size_t z_iter = 0; z_iter < num_fsrs; z_iter++) {
      ...
      for (int i = start_track; i < min_lower; i++) {  //i为轨迹索引
      ...
      if (end_full > start_full) {  // 处理H-tracks的情况，此时没有 V-tracks
      ...
      else if (start_full > end_full) {   // V-tracks的情况，此时没有 H-tracks
 ...
 *        segments
 */
void TraverseSegments::loopOverTracksByStackTwoWay(TransportKernel* kernel) { //使用轨迹堆Z-stack 遍历所有3D轨迹
