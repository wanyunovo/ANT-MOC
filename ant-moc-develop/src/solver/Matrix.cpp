...
namespace antmoc
{
   ...
   *
   * 参数	含义
  cell_locks	OpenMP 互斥锁数组，用于并行写入时的线程安全
  num_x, num_y, num_z	CMFD 网格在三个方向上的单元数
  num_groups	CMFD 粗能群数
  empty_cells	空单元数（六角形网格边角的无效单元，默认为 0）
   */
  Matrix::Matrix(omp_lock_t *cell_locks, int num_x, int num_y, int num_z,
                 int num_groups, int empty_cells)
  {
    ...
    _num_rows = (_num_x * _num_y * _num_z - empty_cells) * _num_groups; // 矩阵的行数 = 有效 CMFD 单元数 × 能群数。这对应 B.29 中的通量向量的维度：每个 CMFD 单元都有e个粗能群 举例：如果有 10×10×5 = 500 个 CMFD 单元，2 个能群，则矩阵有 500×2 = 1000 行

    /* Initialize variables */
    for (int i = 0; i < _num_rows; i++)
      _LIL.push_back(std::map<int, CMFD_PRECISION>()); // 初始化LIL：列表-列表（List of Lists, LIL）表示法，每一行初始化一个空的 map

    _A = NULL;        // CSR 格式的非零值数组
    _IA = NULL;       // CSR 格式的行指针数组
    _JA = NULL;       // CSR 格式的列索引数组
    _DIAG = NULL;     // 对角线元素数组
    _modified = true; // 标记矩阵是否被修改过

    /* Set OpenMP locks for each Matrix cell */
    if (cell_locks == NULL)
      log::ferror("Unable to create a Matrix without an array of cell "
                  "locks");
   ...
   * 参数	物理意义
cell_from	源 CMFD 单元（中子来自哪个单元）
group_from	源能群（中子原来的能量）
cell_to	目标 CMFD 单元（中子去往哪个单元）
group_to	目标能群（中子现在的能量）
val	要累加的值
   */
                              ...
                              int cell_to, int group_to, CMFD_PRECISION val)
  {
    ...
    if (cell_from >= _num_x * _num_y * _num_z || cell_from < 0)
      log::ferror("Unable to increment Matrix value for cell_from %d"
                  " which is not between 0 and %d",
                  cell_from, _num_x * _num_y * _num_z - 1);
    else if (cell_to >= _num_x * _num_y * _num_z || cell_to < 0)
      log::ferror("Unable to increment Matrix value for cell_to %d"
                  " which is not between 0 and %d",
                  cell_from, _num_x * _num_y * _num_z - 1);
    else if (group_from >= _num_groups || group_from < 0)
      log::ferror("Unable to increment Matrix value for group_from %d"
                  " which is not between 0 and %d",
                  group_from, _num_groups - 1);
    else if (group_to >= _num_groups || group_to < 0)
      log::ferror("Unable to increment Matrix value for group_to %d"
                  " which is not between 0 and %d",
                  group_to, _num_groups - 1);

    /* Atomically increment the Matrix value from the
     * temporary array using mutual exclusion locks */
    // 2. 加锁保护
    omp_set_lock(&_cell_locks[cell_to]); // 锁住目标单元

    // 3. 处理 NaN
    if (std::isnan(val))
      val = 0.0;
    // 4. 计算行列索引
    int row = cell_to * _num_groups + group_to;     // 行 = 目标单元×能群数 + 目标能群
    int col = cell_from * _num_groups + group_from; // 列 = 源单元×能群数 + 源能群
                                                    // 5. 累加值到 LIL
    _LIL[row][col] += val;
    // log::fdebug("Insert value %.6f, %d -> %d, (row:%d,col:%d)", val, cell_from, cell_to, row, col);
                        ...
                        int cell_to, int group_to, CMFD_PRECISION val)
  {
    ...
    if (cell_from >= _num_x * _num_y * _num_z || cell_from < 0)
      log::ferror("Unable to set Matrix value for cell_from %d"
                  " which is not between 0 and %d",
                  cell_from, _num_x * _num_y * _num_z - 1);
    else if (cell_to >= _num_x * _num_y * _num_z || cell_to < 0)
      log::ferror("Unable to set Matrix value for cell_to %d"
                  " which is not between 0 and %d",
                  cell_from, _num_x * _num_y * _num_z - 1);
    else if (group_from >= _num_groups || group_from < 0)
      log::ferror("Unable to set Matrix value for group_from %d"
                  " which is not between 0 and %d",
                  group_from, _num_groups - 1);
    else if (group_to >= _num_groups || group_to < 0)
      log::ferror("Unable to set Matrix value for group_to %d"
                  " which is not between 0 and %d",
                  group_to, _num_groups - 1);
  ...
  void Matrix::convertToCSR()
  {
    ...
    log::fverbose_once("Matrix CSR format storage %6.2f MB", (NNZ + _num_rows) * 2 * (sizeof(CMFD_PRECISION) + sizeof(int)) / float(1e6));
    ...
    for (int row = 0; row < _num_rows; row++)
    {
      ...
      for (iter = _LIL[row].begin(); iter != _LIL[row].end(); ++iter)
      {
        if (fabs(iter->second) > FLT_EPSILON)
        {
          ...
          if (row == iter->first)
            _DIAG[row] = iter->second; // 如果是对角线元素，额外记录
  ...
  void Matrix::printString()
  {
    ...
    for (int row = 0; row < _num_rows; row++)
    {
      string << "_DIAG[" << row << "]:" << _DIAG[row] << std::endl;
    }
  ...
  int *Matrix::getIA()
  {
    ...
    if (_modified)    // 如果 LIL 被修改过
      convertToCSR(); // 先转换为 CSR
  ...
  void Matrix::setNumX(int num_x)
  {
    ...
    if (num_x < 1)
      log::ferror("Unable to set Matrix num x to non-positive value %d",
                  num_x);
  ...
  void Matrix::setNumY(int num_y)
  {
    ...
    if (num_y < 1)
      log::ferror("Unable to set Matrix num y to non-positive value %d",
                  num_y);
  ...
  void Matrix::setNumZ(int num_z)
  {
    ...
    if (num_z < 1)
      log::ferror("Unable to set Matrix num z to non-positive value %d",
                  num_z);
  ...
  void Matrix::setNumGroups(int num_groups)
  {
    ...
    if (num_groups < 1)
      log::ferror("Unable to set Matrix num groups to non-positive value"
                  " %d",
                  num_groups);
