...
Matrix::Matrix(omp_lock_t* cell_locks, int num_x, int num_y, int num_z,
  ...
  _num_rows = (_num_x*_num_y*_num_z-empty_cells)*_num_groups;  //代表着这个矩阵的行数

  /* Initialize variables */
  for (int i=0; i < _num_rows; i++)
    _LIL.push_back(std::map<int, CMFD_PRECISION>());  //初始化LIL：列表-列表（List of Lists, LIL）表示法
...
void Matrix::convertToCSR() {
  ...
  log::fverbose_once("Matrix CSR format storage %6.2f MB", (NNZ + _num_rows)
