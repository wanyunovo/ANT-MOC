...
namespace antmoc
{
  ...
  V &FixedHashMap<K, V>::at(K &key)
  {
    ...
    log::error("Key not present in map");
    return iter_node->value;
  }
...
#pragma omp atomic capture
  ...
  {
    log::info("Printing all buckets in the hash map...");
    for (size_t i = 0; i < _M; i++)
    {
      if (_buckets[i] == NULL)
        log::info("Bucket {} -> NULL", i);
      else
        log::info("Bucket {} -> {:p}", i, (void *)_buckets[i]);
    }
  }
...
#pragma omp flush
  ...
  void ParallelHashMap<K, V>::insert(K key, V value)
  {
    ...
    /* get lock hash */
    size_t lock_hash = (std::hash<K>()(key) & (_table->bucket_count() - 1)) % _num_locks;
...
#pragma omp flush
  ...
  template <class K, class V>
  bool ParallelHashMap<K, V>::empty()
  {
    return _table->empty();
  }
...
#pragma omp flush
    ...
    /* get value list */
    V *value_list = table_ptr->values(); // 返回一个新分配的数组，里面是当前表的所有 value，调用者需在用完后释放。
...
#pragma omp flush
  ...
  template class ParallelHashMap<std::string, FSRData *>;
  template class ParallelHashMap<std::string, ExtrudedFSR *>;
