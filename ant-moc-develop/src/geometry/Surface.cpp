...
ZCylinder::ZCylinder(const ZCylinder &zcylinder):
  Surface(zcylinder.getId(), zcylinder.getName()),
...
HexLatticePrism::HexLatticePrism(const double x, const double y,
  ...
  double radius = 0.5 * width_r * std::sqrt(1.0/3.0 + (2*num_r-1)*(2*num_r-1))
                  + 10 * ON_SURFACE_THRESH;
...
HexLatticePrism::HexLatticePrism(const HexLatticePrism &prism):
  Surface(prism.getId(), prism.getName()) {
...
int HexLatticePrism::intersection(Point* point, double azim, double polar,
  ...
  if (distance_to_box != std::numeric_limits<double>::infinity()) {
    ...
    while (_bounding_cylinder->evaluate(&curr) <= ON_SURFACE_THRESH
           && initial_status == _lattice->containsPoint(&curr)) {
