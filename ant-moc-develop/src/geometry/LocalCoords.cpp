...
LocalCoords::LocalCoords(double x, double y, double z, bool first) {
  ...
  _version_num = 0;
  if (first) {
    // 第一个LocalCoords会预先分配内存，用于保存之后的所有LocalCoords
...
void LocalCoords::updateMostLocal(Point* point) {
  ...
  /* 这个函数把链表里最低层的点平移到给定位置，
   * 相应地，链表里其他点也要按相同的向量平移。
   * 使用这个函数时，要保证point与最低层的LocalCoords在同一层上？
   */
...
void LocalCoords::prune() {
  ...
  while (curr != this) {
    ...
    if (curr->getPosition() == -1)
      // 当curr是某个起始LocalCoords时，一次性删掉以它为首的整个链表
      curr->deleteArray();
    curr = next;
  }
...
void LocalCoords::copyCoords(LocalCoords* coords) {
  /* 把当前节点拷贝到传入的coords节点，
   * 把当前节点以下的所有节点拷贝到coords节点下，
   * 完成后，以当前节点和coords节点为首的两个链表里，节点的值相同。
   */
  ...
  while (curr1 != NULL) {
    ...
    if (curr1->getType() == UNIV) {
      // 一个Universe底下只有一个Cell
      curr2->setType(UNIV);
      curr2->setCell(curr1->getCell());
    }
    else {
      // 一个Lattice底下有很多Cell，不能只取一个Cell
