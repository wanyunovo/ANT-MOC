...
namespace antmoc
{
  ...
  ///          first ID begins at 1000000. Hence, user-defined Cell IDs greater
  ///          than or equal to 1000000 are prohibited.
  int cell_id()
  {
    int id = auto_id;
    auto_id++;
                              ...
                              std::string units)
  {
    if (num_axes != 3)
      log::ferror("Unable to get rotation with %d axes for Cell %d. "
                  "The rotation array should be length 3.",
                  num_axes, _id);
  ...
  void Cell::retrieveTranslation(double *translations, int num_axes)
  {
    if (num_axes != 3)
      log::ferror("Unable to get translation with %d axes for Cell %d. "
                  "The translation array should be length 3.",
                  num_axes, _id);
  ...
  /**
   * 只在 Cell 类型是 FILL 并且 _fill 非空时工作：它把 _fill 当成 Universe 指针，先把该 Universe 自己加入 map，然后根据 getType() 判断是普通 Universe 还是 Lattice——两者都继承自 Universe，所以都能递归调用 getAllUniverses()，把更深层的 Universe 继续插入 map。
   */
  ...
  void Cell::setTranslation(double *translation, int num_axes)
  {
    ...
    if (_cell_type == MATERIAL)
      log::ferror("Translations cannot be defined on material cells, "
                  ...
                  "the current material cell ID %d.",
                  _id);
  ...
  /// \brief 将一个表面插入到该单元的边界表面容器中。
  /// \param halfspace 表面半空间 (+/-1)
  /// \param surface 指向 Surface 的指针
  void Cell::addSurface(int halfspace, Surface *surface)
  {
    ...
    if (halfspace != -1 && halfspace != +1)
      log::ferror("Unable to add surface %d to cell %d since the halfspace"
                  " %d is not -1 or 1",
                  surface->getId(), _id, halfspace);

    // 假设是与现有区域的交叉点。
    // 如果这是一个简单的单元格，则 _rpn 中没有运算符。
  ...
  double Cell::minSurfaceDist(Point *point, double azim, double polar)
  {
    ...
    for (auto &s_pair : _surfaces) //_surfaces 是这个 Cell 边界面（Surface）的容器，元素类型是 std::pair<int, Surface*>。函数遍历每一个 surface：
    {
  ...
  void Cell::ringify(std::vector<Cell *> &subcells, double max_radius)
  {
    ...
    for (auto hs : *this)
    {
      ...
      if (surface->getSurfaceType() == ZCYLINDER)
      {
        ...
        ZCylinder *zcylinder = static_cast<ZCylinder *>(surface); // 将基类指针转换为派生类指针
    ...
    if (num_zcylinders > 2)
      log::ferror("Unable to ringify Cell %d since it "
                  "contains more than 2 ZCYLINDER Surfaces",
                  _id);
    ...
    if (radius1 <= radius2)
      log::ferror("Unable to ringify Cell %d since it contains 2 "
                  ...
                  "the 2 halfspaces for each Surface.",
                  _id, halfspace1,
  ...
  // 先让自己关联的所有 Surface 知道“我就是邻居之一”，再沿着填充层次向下递归，让嵌套 Universe/Lattice 内部的 Cell 也完成同样的邻居登记，从而最终构建整棵几何结构的邻接关系。
  void Cell::buildNeighbors()
  {

    /* Add this Cell to the neighbor lists of all this cell's surfaces */
    for (auto hs : *this)                              // 使用 CellIter 遍历该 Cell 的全部半空间（每个半空间对应一个面和 +/−1 符号）
      hs.surface->addNeighborCell(hs.halfspace, this); // 把当前 Cell 注册到该 Surface 的邻居列表里。半空间符号告诉 Surface 这个 Cell 位于曲面的哪一侧，便于射线穿越时判断“另一边的 Cell 是谁”。
