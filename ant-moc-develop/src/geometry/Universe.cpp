...
namespace antmoc
{
  ...
  boundaryType Universe::getMinXBoundaryType()
  {
    ...
    if (_min_x_bound != VACUUM && _min_x_bound != INTERFACE)
      log::error("The code was compiled specially for cases with only "
                 ...
                 "boundary condition was found in universe {}.",
                 _id);
#endif
  ...
  Cell *Universe::getCell(int cell_id)
  {
    ...
    if (_cells.find(cell_id) == _cells.end())
    {
      log::ferror("Unable to return Cell with ID = %d from Universe with "
                  "ID = %d since it does not contain this Cell",
                  cell_id, _id);
    }
   ...
   *  先调用 getAllCells() 把自己和所有嵌套层级里的 Cell* 都拿到，然后把当前 Universe (_id, this) 记入 map，接着遍历每个 Cell，调用 cell->getAllUniverses() 收集“这个 Cell 里再填充的 Universe/Lattice”，把结果合并进 map。
   */
   ...
   *
   * 本层universe cells 填充cell的lattice或universe
   */
  Cell *Universe::findCell(LocalCoords *coords)
  {
    ...
    /* Loop over all Cells */
    std::map<int, Cell *>::iterator iter;
    // 遍历这个 Universe 里的每个 Cell
    for (iter = _cells.begin(); iter != _cells.end(); ++iter)
    {
      Cell *cell = iter->second;

      if (cell->containsCoords(coords)) // 注意：一个点只会在一个cell中
      {
        ...
        else if (cell->getType() == FILL)
        {
          ...
          // 如果 Cell 本身带平移 / 旋转，就把 next_coords 的坐标做变换
   ...
   *
   * 为了优化特征线跟踪,为 Universe 内所有 Cell 建立邻居 Cell  集合
   */
