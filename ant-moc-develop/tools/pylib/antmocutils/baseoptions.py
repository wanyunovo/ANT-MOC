...
class BaseOpt(object):
    ...
    Traceback (most recent call last):
        ...
    AttributeError: A BaseOpt object must have a name or a shortname
    ...
    def __init__(self, name="", shortname="", default=None, sep="", isbool=False, valspec="", doc=""):

        super(BaseOpt, self).__setattr__('_attrs', collections.OrderedDict([
            ("name",       name),
            ("shortname",  shortname),
        ...
        if not self.name and not self.shortname:
            raise AttributeError("A BaseOpt object must have a name or a shortname")

        if len(self.shortname) > 1:
            raise AttributeError("Shortname of a BaseOpt object must a single character")
        ...
        try:
            if name in ["value", "default"]:
                if self._attrs["isbool"]:
                    return self._attrs[name] != None
                elif self._attrs[name] is None:
    ...
    def get_longarg(self):
        if self.name and not self.isbool:
            return '{}='.format(self.name)
        else:
            return self.name

    def get_shortarg(self):
        if self.shortname and not self.isbool:
            return '{}:'.format(self.shortname)
        else:
            return self.shortname

    def matchArg(self, arg):
...
class BaseOptions(object):
    ...
    in its constructor to add a list of BaseOpts.

    See antmoclog.options.LogOptions for an exhaustive example.
    ...
    def __getitem__(self, name):
        for names in self.opts.keys():
            if name in names:
                return self.opts[names]
        raise KeyError("Option '{}' not found".format(name))

    def __str__(self):
        return str(self.opts.items())

    def add_opts(self, opts):
        """Extract keys to generate a dictionary."""

        for opt in opts:
            if isinstance(opt, BaseOpt):
                self.opts[opt.names()] = opt
            else:
                raise TypeError("opts must be a list of BaseOpt")
    ...
    def get_shortargs(self):
        return [i.get_shortarg() for i in self.opts.values() if i.get_shortarg()]

    def get_longargs(self):
        return [i.get_longarg() for i in self.opts.values() if i.get_longarg()]
