...
class TinyExtractor(object):
    ...
    >>> logdb = antmoclog.data.LogDB()

    >>> extractor = TinyExtractor(logdb = logdb)

    >>> extractor.setup(LogOptions())

    """

    def __init__(self, logdb, specs = None, sortby = "", sep = " | ", output = "antmoc.records.csv", truncate = False, summary = False):
        self.logdb      = logdb
        self.specs      = specs
        self.sortby     = sortby
        self.sep        = sep
    ...
    def extract(self):
        ...
        specs = antmoclog.fields.expand_specs(self.specs)
