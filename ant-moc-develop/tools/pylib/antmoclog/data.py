...
class LogFile(object):
    ...
    See package antmoclog.fields for more details.
    ...
    def save(self):
        ...
        """
        self.dump_to(path = self.path, mode = "w")
    ...
    def findall(self, specs):
        ...
        ----------
        specs : list
            List of field specs, see antmoclog.fields.FieldSpec for more details.
        ...
        # Expand perl regex
        specs = antmoclog.fields.expand_specs(specs)
...
class LogDB(object):
    ...
    def setup(self, options):
        ...
        """
        if isinstance(options, antmoclog.options.LogOptions):
    ...
    def rm_paths(self, patterns):
        """Remove file paths matching given patterns."""
        pass

    def dump_to_file(self, path, directory):
        ...
        if filepath.exists():
            ...
            logfile.dump_to(filepath.resolve(), mode = "x")
            return True

    def dump_to(self, directory):
        ...
        with multiprocessing.Pool(processes = self.nprocs) as pool:
            are_dumped = pool.starmap(self.dump_to_file, zip(arg_paths, arg_dirs))

        n_dumped  = sum([1 for x in are_dumped if x])
        n_skipped = len(are_dumped) - n_dumped

        print("LogDB: skipped {} file(s), saved {} file(s) to '{}'".format(n_skipped, n_dumped, path.resolve()))
    ...
    def query(self, specs, sortby = ""):
        ...
        # properly.
        logfields = antmoclog.fields.LogFields()
        if sortby:
