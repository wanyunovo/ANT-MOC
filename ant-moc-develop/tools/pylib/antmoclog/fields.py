...
Classes: Field, LogFields, FieldSpec, FieldPred, FieldEncoder, FieldsEncoder

Functions: helpFields, expandSpecs, dump, load
...
class Field(object):
    ...
    def __init__(self, name, dtype = str, fmt = "{}", patterns = [], doc = ""):
        ...
        if not isinstance(dtype, type):
            raise TypeError("dtype '{}' is not a type object".format(dtype))

        if not isinstance(fmt, str):
            raise TypeError("fmt '{}' is not a string")
...
class LogFields(object):
    ...
    def __new__(cls):
        if cls.instance is None:
            ...
            # Read default fields
            import inspect, os
            path = os.path.dirname(inspect.getfile(antmoclog.fields))
...
class FieldSpec(object):
    ...
    def __init__(self, spec):
        ...
        if re_matched:
            ...
            raise ValueError("Failed to unpack field spec '{}'".format(spec))

        # Check if value comes along with op
        if op and not value or not op and value:
            raise ValueError("An op or value is missing in field spec '{}'".format(spec))
    ...
    def expanded(self):
        ...
        FieldPred object to create a new spec object.
        ...
        re_field = re.compile("^{}$".format(self.name))
        ...
        for field_name in LogFields().names():
            ...
            if re_matched:
                ...
                spec      = FieldSpec("{}{}{}".format(re_matched.group(), self.op, self.value))
                spec.pred = FieldPred(name=re_matched.group(), op = self.op, value = self.value)
                specs.append(spec)

        if not specs:
            raise KeyError("No field name matches pattern '{}'".format(self.name))

        return specs


class FieldPred(object):
    ...
    def __init__(self, name, op = "", value = ""):
        ...
        if self.op in ["<", ">"]:
            try:
                ...
                raise AttributeError("Value '{}' is not of the dtype of field '{}'".format(self.value, name))
    ...
    def __call__(self, value):
        ...
        try:
            ...
            raise AttributeError("Value '{}' is not of the dtype".format(value))
