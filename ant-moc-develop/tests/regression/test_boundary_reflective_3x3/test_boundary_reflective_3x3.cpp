/// \file test_boundary_reflective_3x3.cpp
/// \date 2020/03
/// \author An Wang, USTB (wangan@xs.ustb.edu.cn)
/// \purpose 回归测试。测试所有边界均为全反射边界的3x3棒束。
///          轨迹密度的设置保证追踪出全部216个FSR。
///          在KeffFromNeutronBalance测试中，结果与基本条件的测试
///          不一样，这一点不同于vacuum条件和mixed条件的测试。
...
///          Tracing: OTF Stacks
TEST_F(test_boundary_reflective_3x3, EqualWeight) {
...
TEST_F(test_boundary_reflective_3x3, OTFTracks) {
...
TEST_F(test_boundary_reflective_3x3, modules3x3) {
...
TEST_F(test_boundary_reflective_3x3, KeffFromNeutronBalance) {
...
TEST_F(test_boundary_reflective_3x3, EqualAngle) {
...
TEST_F(test_boundary_reflective_3x3, GaussLegendre) {
...
TEST_F(test_boundary_reflective_3x3, TabuchiYamamoto) {
...
TEST_F(test_boundary_reflective_3x3, Leonard) {
...
TEST_F(test_boundary_reflective_3x3, compressedXSLayout) {
